...
async function syncParentDirectory(filePath, fsApi) {
  let handle;
  try {
...
async function writeSettingsAtomic(filePath, contents, fsApi = fs) {
...
export async function saveCloudSettings(userDataPath, payload = {}, { fsApi = fs } = {}) {
  ...
  await writeSettingsAtomic(settingsPath(userDataPath), `${JSON.stringify(next, null, 2)}\n`, fsApi);
  return publicSettings(next, userDataPath);
}

export async function clearCloudSettings(userDataPath, { fsApi = fs } = {}) {
