...
async function handleDesktopApiRequest(request) {
  return await handleDesktopCloudRequest(request, app.getPath('userData'), net)
    || await handleDesktopLocalApiRequest(request, net);
}
...
function installIpc() {
  ...
  ipcMain.handle(IPC.getCloudSettings, (event) => {
    requireTrustedSender(event);
    return readCloudSettings(app.getPath('userData'));
  });
  ipcMain.handle(IPC.saveCloudSettings, (event, payload) => {
    requireTrustedSender(event);
    return saveCloudSettings(app.getPath('userData'), payload);
  });
  ipcMain.handle(IPC.clearCloudSettings, (event) => {
    requireTrustedSender(event);
    return clearCloudSettings(app.getPath('userData'));
  });
  ...
  const template = [
    ...
    }] : []),
    {
      ...
      submenu: [
        ...
        { label: 'Cloud Settings...', accelerator: 'CmdOrCtrl+,', click: () => sendMenuCommand(MENU_COMMAND.showCloudSettings) },
...
if (!app.requestSingleInstanceLock()) {
  ...
  app.on('before-quit', (event) => {
    if (converterArtifactsReleasedForQuit) return;
    event.preventDefault();
    if (releasingConverterArtifactsBeforeQuit) return;
    releasingConverterArtifactsBeforeQuit = true;
    void converterJobs.shutdown()
      .then(() => converterJobs.releaseTerminalArtifacts())
      .catch(error => console.error('[converter-jobs] shutdown failed', error))
      .finally(() => {
        converterArtifactsReleasedForQuit = true;
        app.quit();
      });
  });
  ...
  app.whenReady().then(async () => {
    ...
    registerStaticProtocol({
      ...
      scheme: APP_SCHEME,
      rootDir: ROOT_DIR,
      handleApiRequest: handleDesktopApiRequest,
    });
    installCsp();
    installIpc();
