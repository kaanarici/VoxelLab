...
const IPC = Object.freeze({
  ...
  getCloudSettings: 'desktop:get-cloud-settings',
  saveCloudSettings: 'desktop:save-cloud-settings',
  clearCloudSettings: 'desktop:clear-cloud-settings',
...
contextBridge.exposeInMainWorld('voxellabDesktop', {
  ...
  getCloudSettings: () => ipcRenderer.invoke(IPC.getCloudSettings),
  saveCloudSettings: (payload = {}) => ipcRenderer.invoke(IPC.saveCloudSettings, {
    modalWebhookBase: String(payload.modalWebhookBase || ''),
    modalAuthToken: String(payload.modalAuthToken || ''),
    r2PublicUrl: String(payload.r2PublicUrl || ''),
    trustedUploadOrigins: Array.isArray(payload.trustedUploadOrigins)
      ? payload.trustedUploadOrigins.map(String)
      : String(payload.trustedUploadOrigins || ''),
    cloudProcessing: payload.cloudProcessing !== false,
    clearModalAuthToken: payload.clearModalAuthToken === true,
  }),
  ...
  readFileRange: (filePath, range = {}) => {
    ...
    return ipcRenderer.invoke(IPC.readFileRange, String(filePath || ''), {
