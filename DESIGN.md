...
Machine-readable design contract for VoxelLab — a local-first research viewer for medical image volumes (not for clinical use). Agents read this file to build new UI that is on-brand without re-deriving intent from the CSS. It documents **why** each decision exists, not just the values.

Source of truth: design tokens in `css/base.css :root` (+ `html.light`), and the per-surface CSS classes across `css/*.css` (base, sidebar, toolbar, panels, viewer, modals, command-palette). The UI is **template-driven**: DOM is authored in `templates/*.html` and wired by thin JS in `js/<domain>/`. Vanilla ESM, no bundler, no framework, statically deployable. When code and this file disagree, the code wins — update this file.
...
Two consequences agents must preserve:
