...
import { drawMicroscopyChannelComposite, ensureMicroscopyComposite } from './microscopy-channel-composite.js';
...
import { updateScaleBar } from './scale-bar.js';
import {
  initMprView,
  drawMPR,
  drawMPRInteractive,
...
function microscopyCompositeSources(series) {
  ...
  if (!composite.enabled || sizeC <= 1 || !stacks) {
    ...
    const channel = series.microscopyDataset?.channels?.find((item) => Number(item?.index) === c) || { index: c };
    return bytes ? [{
      index: c,
