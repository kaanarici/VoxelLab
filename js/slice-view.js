...
import { channelByIndex } from './microscopy/microscopy-dataset-model.js';
import {
  initMprView,
  drawMPR,
  drawMPRInteractive,
...
function microscopyCompositeSources(series) {
  if (series?.imageDomain !== 'microscopy') return [];
  const sizeC = Math.max(1, Number(series.microscopy?.sizeC || series.microscopyDataset?.channels?.length || 1));
  const composite = ensureMicroscopyComposite(series, sizeC);
  const stacks = state._localMicroscopyStacks?.[series.slug];
  const t = Math.max(0, Math.floor(Number(series.microscopy?.timeIndex || 0)));
  const sources = [];
  if (!composite.enabled || sizeC <= 1 || !stacks) {
    const c = Math.max(0, Math.floor(Number(series.microscopy?.channelIndex || 0)));
    const displayRange = state.imgs?.[state.sliceIdx]?._microscopyDisplayByteRange || null;
    if (!displayRange) return [];
    const bytes = readImageByteData(state.imgs[state.sliceIdx], series.width, series.height);
    const channel = channelByIndex(series.microscopyDataset, c) || { index: c };
    return bytes ? [{
      index: c,
      channel: channel.color || channel.displayColor ? channel : { ...channel, color: '#FFFFFF' },
      bytes,
      displayRange,
    }] : [];
  }
  for (let c = 0; c < sizeC; c += 1) {
    if (composite.channels[c] === false) continue;
    const img = stacks[`${c}|${t}`]?.[state.sliceIdx];
    const bytes = readImageByteData(img, series.width, series.height);
    if (!bytes) continue;
    const channel = channelByIndex(series.microscopyDataset, c) || { index: c };
    sources.push({ index: c, channel, bytes, displayRange: img?._microscopyDisplayByteRange || null });
  }
  return sources;
}
...
export function drawSlice() {
  ...
  const drewMicroscopyComposite = microscopyComposite.length > 0 && drawMicroscopyChannelComposite(ctx, series.width, series.height, {
    sources: microscopyComposite,
    window: state.window,
    level: state.level,
    invert: state.invertDisplay,
  });

  if (!drewMicroscopyComposite && !anyOverlay) {
  ...
  const wlW = $('wl-window'); if (wlW) wlW.value = state.window;
  const wlL = $('wl-level'); if (wlL) wlL.value = state.level;
