  ...
  return new THREE.RawShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        if (uHasSeg == 1) {
          float seg = floor(projectDiscreteLabel(uSeg, coord) * 255.0 + 0.5);
        ...
        if (uHasRegions == 1) {
          float region = floor(projectDiscreteLabel(uRegions, coord) * 255.0 + 0.5);
...
export function drawGpuMprSlice(canvas, {
  ...
  try {
    ...
    if (lutCache.grayKey !== wlLut.key || lutCache.grayR !== wlLut.r || lutCache.grayG !== wlLut.g || lutCache.grayB !== wlLut.b) {
      fillGrayLutBytes(lutCache.grayBytes, wlLut);
      lutCache.grayR = wlLut.r;
      lutCache.grayG = wlLut.g;
      lutCache.grayB = wlLut.b;
      lutCache.grayKey = wlLut.key;
