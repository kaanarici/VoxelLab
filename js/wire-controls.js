...
import { drawCompare, loadComparePeers, buildCompareMenu, buildCompareGrid, getGroupPeers } from './series/compare.js';
import {
  toggleAnnotate,
...
export function wireControls(deps) {
  ...
  const closePopups = () => {
    ...
    openPopupContainer = null;
    if (popupPlaceRaf) cancelAnimationFrame(popupPlaceRaf);
    popupPlaceRaf = 0;
    cmapTrigger.setAttribute('aria-expanded', 'false');
  ...
  cmapTrigger.addEventListener('click', (e) => {
    ...
    if (!wasOpen) {
      ...
      placePopup(cmapDropdown);
      openPopupContainer = cmapDropdown;
    }
  });
  ...
  toolRail?.addEventListener('scroll', () => {
    ...
    popupPlaceRaf = requestAnimationFrame(() => {
      popupPlaceRaf = 0;
      if (openPopupContainer?.classList.contains('open')) placePopup(openPopupContainer);
    });
  }, { passive: true });
  ...
  const syncClearSliceButton = () => {
    ...
    const has = !!series && drawingCountsForSlice(state, series, state.sliceIdx).total > 0;
    btn.hidden = !has;
  ...
  document.querySelectorAll('.toolbox-trigger').forEach((trigger) => {
    trigger.addEventListener('click', (e) => {
      ...
      if (!wasOpen) {
        ...
        if (box.querySelector('#btn-clear')) syncClearSliceButton();
        placePopup(box);
        openPopupContainer = box;
  ...
  const syncToolboxBadges = () => {
    document.querySelectorAll('.toolbox').forEach((box) => {
      ...
      const anyActive = [...panel.querySelectorAll('.icon-btn')].some((b) =>
        b.classList.contains('active') && !b.hidden && !b.classList.contains('hidden'));
  ...
  initSlimSAMTool({ drawSlice, beforeActivate: () => deactivateOthers('slimsam'), selectSeries });
  ...
  $('btn-anatomy-labels').onclick = toggleAnatomyLabels;
  for (const [id, fmt] of [['label-export-study-stl', 'stl'], ['label-export-study-obj', 'obj']]) {
    $(id)?.addEventListener('click', () => {
      void import('./mesh/mesh-export.js').then((m) => m.exportStudyMesh(state.manifest?.series?.[state.seriesIdx], fmt, { notify: true }));
    });
  }
  ...
  const openComparePicker = ({ focusFirst = false, autoStart = false } = {}) => {
    ...
    placePopup(cmpDropdown);
    openPopupContainer = cmpDropdown;
  ...
  window.addEventListener('voxellab:open-registration-compare', async (e) => {
    const referenceSlug = String(e.detail?.referenceSlug || '').trim();
    const movingSlug = String(e.detail?.movingSlug || '').trim();
    if (!referenceSlug || !movingSlug || referenceSlug === movingSlug) return;
    const seriesList = state.manifest?.series || [];
  ...
  bind('s-low', (v) => { setVolumeTransfer({ lowT: v }); syncCTWindowActive(); });
  bind('s-high', (v) => { setVolumeTransfer({ highT: v }); syncCTWindowActive(); });
  bind('s-gain', (v) => { setVolumeTransfer({ intensity: v }); });
  bind('s-anatomy-color', (v) => { setOverlayOpacity(v); });
  ...
  const applyTransferShortcut = (preset) => {
    applyPreset(preset);
    syncCTWindowActive();
  };
  $('preset-full').onclick = () => applyTransferShortcut({ lowT: 0, highT: 1.0, clipMin: [0, 0, 0], clipMax: [1, 1, 1] });
  $('preset-surface').onclick = () => applyTransferShortcut({ lowT: 0.25, highT: 1.0, clipMin: [0, 0, 0], clipMax: [1, 1, 1] });
  $('preset-inside').onclick = () => applyTransferShortcut({ lowT: 0, highT: 0.6, clipMin: [0, 0, 0], clipMax: [1, 1, 1] });
  $('preset-halfx').onclick = () => applyTransferShortcut({ lowT: state.lowT, highT: state.highT, clipMin: [0, 0, 0], clipMax: [0.5, 1, 1] });
  $('preset-halfy').onclick = () => applyTransferShortcut({ lowT: state.lowT, highT: state.highT, clipMin: [0, 0, 0], clipMax: [1, 0.5, 1] });
  $('preset-reset').onclick = () => applyTransferShortcut({ lowT: 0.08, highT: 1.0, intensity: 1.6, clipMin: [0, 0, 0], clipMax: [1, 1, 1] });
