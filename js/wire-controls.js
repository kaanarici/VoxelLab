...
export function wireControls(deps) {
  ...
  initSlimSAMTool({ drawSlice, beforeActivate: () => deactivateOthers('slimsam') });
  ...
  for (const [id, fmt] of [['label-export-study-stl', 'stl'], ['label-export-study-obj', 'obj']]) {
    $(id)?.addEventListener('click', () => {
      void import('./mesh/mesh-export.js').then((m) => m.exportStudyMesh(state.manifest?.series?.[state.seriesIdx], fmt));
    });
  }
