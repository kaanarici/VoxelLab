...
export function wireControls(deps) {
  ...
  document.getElementById('tool-rail')?.addEventListener('scroll', closePopups, { passive: true });
  ...
  const syncClearSliceButton = () => {
    ...
    const has = !!series && drawingCountsForSlice(state, series.slug, state.sliceIdx).total > 0;
    btn.hidden = !has;
