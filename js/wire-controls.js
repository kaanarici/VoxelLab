...
import { is3dActive, isMprActive } from './mode-flags.js';
...
import { handleLocalImport, showStudyUploadModal } from './study-upload-modal.js';
...
import { syncMrPresetActiveState, syncToolbarReadyState } from './toolbar-chrome.js';
...
export function wireControls(deps) {
  ...
  wireDesktopBridge(selectSeries);
  ...
  const openComparePicker = ({ focusFirst = false } = {}) => {
    if (state.manifest.series.length < 2) return;
    buildCompareMenu(cmpMenu, {
      onSelectionChanged({ peers }) {
        if (state.mode === 'cmp' && peers.length < 2) toggleCompare();
      },
    });
    const wasOpen = cmpDropdown.classList.contains('open');
    closePopups();
    if (wasOpen && !focusFirst) return;
  ...
  cmpButton.addEventListener('click', (e) => {
    ...
    if (peers.length >= 2) {
      ...
      cmpButton.setAttribute('aria-expanded', 'false');
      toggleCompare();
...
function clampDesktopRange(value, fallback, size) {
  const number = Number(value);
  if (!Number.isFinite(number)) return fallback;
  return Math.max(0, Math.min(size, Math.trunc(number)));
}
...
class DesktopPathFile extends DesktopPathBlob {
  ...
  slice(start = 0, end = this.size) {
    const rangeStart = clampDesktopRange(start, 0, this.size);
    const rangeEnd = clampDesktopRange(end, this.size, this.size);
    return new DesktopPathBlob(this, Math.min(rangeStart, rangeEnd), Math.max(rangeStart, rangeEnd));
  }
}

function desktopFileFromRecord(record, desktop) {
  return new DesktopPathFile(record, desktop);
}

function wireDesktopBridge(selectSeries) {
  ...
  desktop.onOpenPaths(async (payload) => {
    try {
      ...
      const filePaths = (payload?.supported || [])
      ...
      if (!filePaths.length) {
        if (unsupported.length) {
          showDialog(
            ...
            escapeHtml(`VoxelLab cannot open: ${unsupported.map(item => item.name).join(', ')}`),
          );
        }
      ...
      await showStudyUploadModal(selectSeries);
      const files = (payload.supported || [])
        .filter(record => record.kind === 'file')
        .map(record => desktopFileFromRecord(record, desktop));
      await handleLocalImport(files, $('upload-status'), $('upload-modal'), selectSeries);
    } catch (e) {
