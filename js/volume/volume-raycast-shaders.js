...
export const VOLUME_RAYCAST_FRAGMENT_SHADER = /* glsl */`
      ...
      uniform sampler3D uGrad;  // precomputed normal+edge: rgb = n*0.5+0.5, a = clamp(|grad|*8,0,1)
      uniform int   uHasGrad;   // 1 = sample uGrad instead of the 6-tap fallback
      varying vec3 vOrigin;
      varying vec3 vDir;
        ...
        if (uMode == 0) {
          ...
          for (float i = 0.0; i < 2048.0; i++) {
            ...
            if (raw >= uLowT && raw <= uHighT) {
              ...
              // Surface normal + edge term. Prefer the precomputed gradient
              // volume (one fetch); fall back to the 6-tap central difference
              // while that texture is still being built off-thread.
              vec3 nrm;
              float edge;
              if (uHasGrad == 1) {
                vec4 g = texture(uGrad, p);
                nrm = g.rgb * 2.0 - 1.0;   // unit normal, or ~0 in flat regions
                edge = g.a;                // baked edge term; exactly 0 on flat voxels
              } else {
                vec3 grad = gradient(p);
                float gmag = length(grad);
                nrm = gmag > 1e-4 ? grad / gmag : vec3(0.0);
                edge = clamp(gmag * 8.0, 0.0, 1.0);
              }

              // Gradient-based Phong lighting. Flat regions get a near-1.0
              // brightness floor so homogeneous tissue isn't dimmed, only
              // actual surfaces get directional shading. Gate on the edge term:
              // it bakes to exactly 0 on flat voxels, whereas the baked normal
              // floors at byte 128 (~0.004) and can never read as a true zero.
              if (edge > 0.0 && dot(nrm, nrm) > 1e-6) {
                // Re-normalize: the baked normal is unit per-voxel, but linear
                // filtering between a surface and a flat voxel shortens it.
