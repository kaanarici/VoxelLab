...
const IMPERIAL_LENGTH_UNITS = new Set(['inch', 'foot', 'yard', 'mile']);
...
export function isMetricLengthUnit(unit = '') {
  const key = unitKey(unit);
  if (!isKnownLengthUnit(key)) return false;
  const normalized = UNIT_LABELS.get(key) || key;
  return !IMPERIAL_LENGTH_UNITS.has(key) && !IMPERIAL_LENGTH_UNITS.has(normalized);
}
