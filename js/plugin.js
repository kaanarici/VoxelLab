...
import { state, subscribe, getStateSnapshot } from './core/state.js';
import { $ } from './dom.js';
...
export { addOverlay, drawPluginOverlays } from './plugin-overlays.js';
...
function bindContextMenu() {
  ...
  canvas.addEventListener('contextmenu', (event) => {
    ...
    const visibleItems = [...contextMenuRegistry.values()].filter((item) => {
      try {
        return item.condition(snapshot);
      } catch {
        return false;
      }
    });
