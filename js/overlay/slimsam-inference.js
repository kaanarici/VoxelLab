...
export function configureSlimSAMOrtRuntime(ort) {
  ...
  _sessionLoading = (async () => {
    try {
      if (!_ort) {
        ...
        configureSlimSAMOrtRuntime(_ort);
      }
...
export async function slimsamRunDecoder(session, embedBuf, meta, sliceIdx, clickX, clickY) {
  ...
  const binaryMask = slimsamResizeLogits(rawMask, maskW, maskH, width, height);

  return { mask: binaryMask, width, height };
}

export function slimsamResizeLogits(rawMask, maskW, maskH, width, height, threshold = 0) {
  const binaryMask = new Uint8Array(width * height);
  if (maskW === width && maskH === height) {
    for (let i = 0; i < binaryMask.length; i++) binaryMask[i] = rawMask[i] > threshold ? 255 : 0;
    return binaryMask;
  }

  for (let row = 0; row < height; row++) {
    const srcY = Math.max(0, Math.min(maskH - 1, (row + 0.5) * maskH / height - 0.5));
    const y0 = Math.floor(srcY);
    const y1 = Math.min(y0 + 1, maskH - 1);
    const wy = srcY - y0;
    for (let col = 0; col < width; col++) {
      const srcX = Math.max(0, Math.min(maskW - 1, (col + 0.5) * maskW / width - 0.5));
      const x0 = Math.floor(srcX);
      const x1 = Math.min(x0 + 1, maskW - 1);
      const wx = srcX - x0;
      const top = rawMask[y0 * maskW + x0] * (1 - wx) + rawMask[y0 * maskW + x1] * wx;
      const bot = rawMask[y1 * maskW + x0] * (1 - wx) + rawMask[y1 * maskW + x1] * wx;
      binaryMask[row * width + col] = (top * (1 - wy) + bot * wy) > threshold ? 255 : 0;
    }
  }
  return binaryMask;
}
