...
import { setAnnotateMode } from '../core/state/viewer-tool-commands.js';
import {
...
export function getAnnotationsHere() {
  return noteEntriesForSlice(state, state.manifest.series[state.seriesIdx], state.sliceIdx);
}

export function setAnnotationsHere(list) {
  setNoteEntriesForSlice(state, state.manifest.series[state.seriesIdx], state.sliceIdx, list);
}
...
export function annotationPinsForSlice(seriesOrSlug, sliceIdx) {
  const series = seriesOrSlug || state.manifest.series[state.seriesIdx];
  return noteEntriesForSlice(state, series, sliceIdx);
}
...
export function drawAnnotationPins(ctx, {
  ...
  const list = annotationPinsForSlice(series || slug, sliceIdx);
...
export function renderAnnotationList() {
  ...
  if (!host) return;
  const series = state.manifest.series[state.seriesIdx];
  const entries = drawingEntriesForSeries(state, series)
  ...
  host.querySelectorAll('.annot-row').forEach(el => {
    el.addEventListener('click', (e) => {
      ...
      if (e.target.classList.contains('ar-del')) {
        e.stopPropagation();
        const next = deleteDrawingEntryById(noteEntriesForSlice(state, series, z), +el.dataset.id);
        setNoteEntriesForSlice(state, series, z, next);
        renderAnnotationList();
        _drawSlice();
        _drawSparkline();
        return;
