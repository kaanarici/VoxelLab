...
async function submitAskQuestion() {
  ...
  // (multiple regions and/or context studies) → a study ask whose question names
  // the focus regions + context studies; the agent reads/crops them itself.
  ...
  try {
    const r = await fetch('/api/ask/stream', {
      ...
      body: JSON.stringify({
        slug: ctx.slug, slice: ctx.slice, region, question: q,
