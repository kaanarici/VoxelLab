...
export function mprPlaneSizes(series) {
  ...
  const spanPixels = (count, spacing, referenceSpacing) => (
    count > 1 ? 1 + ((count - 1) * spacing / referenceSpacing) : 1
  );
  return {
    axW: W,
    axH: clamp(Math.round(spanPixels(H, rowSpacing, colSpacing))),
    coW: W,
    coH: clamp(Math.round(spanPixels(D, zSpacing, colSpacing))),
    saW: H,
    saH: clamp(Math.round(spanPixels(D, zSpacing, rowSpacing))),
  };
}
