...
function rowNode(row, host = state) {
  ...
  } else if (row.valueSource === 'raw_scalar') {
    footText = 'Raw scalar intensity';
...
export function renderRoiResults(host = state) {
  ...
  const analysisOps = analysisIdentity ? host?._microscopyAnalysisLog?.[analysisIdentity] || [] : [];
  ...
  if (evidenceExportButton) evidenceExportButton.disabled = series?.imageDomain !== 'microscopy' || !analysisOps.length;
