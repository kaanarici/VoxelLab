...
export const MICROSCOPY_EVIDENCE_LIMITATIONS = Object.freeze([
  ...
  'Colocalization reports raw pixel-wise Pearson plus thresholded Manders tM1/tM2 only; it has no Costes/randomization/significance or object-based analysis.',
  'Background correction and bleed-through controls remain the researcher\'s responsibility.',
...
function analysisOpsForSeries(host, series) {
  ...
  const ops = identity ? host?._microscopyAnalysisLog?.[identity] || [] : [];
...
function intensityDomains(rows = []) {
  ...
  for (const row of rows) {
    ...
    const current = bySource.get(source) || {
      ...
      rows: 0,
      meaning: source === 'raw_16bit' || source === 'raw_scalar'
...
export function microscopyEvidencePackageFailureText(reason = '') {
  ...
  if (reason === 'no_analysis_descriptor') return 'Run a supported microscopy analysis before exporting an evidence package';
