...
function roiStatsForExport(stats = {}) {
  ...
  if (stats.valueSource === 'raw_16bit') {
    return {
...
function valuesForEntry(entry, stats) {
  if (entry.kind === 'line' || entry.kind === 'angle') {
    return {
      mean: null,
      std: null,
      min: null,
...
export function setRoiResultLabel(row, label, host = state) {
  ...
  if (row.kind === 'angle') {
    const list = angleEntriesForSlice(host, series.slug, sliceIdx);
    const item = list.find(entry => Number(entry?.id) === id);
    if (!item) return false;
    item.label = clean;
    setAngleEntriesForSlice(host, series.slug, sliceIdx, list);
    return true;
  }
  if (row.kind === 'line') {
...
export function roiResultRows(host = state, series = host?.manifest?.series?.[host.seriesIdx]) {
  ...
  return drawingEntriesForSeries(host, series.slug)
    .filter((entry) => entry.kind === 'ellipse' || entry.kind === 'polygon' || entry.kind === 'point' || entry.kind === 'line' || entry.kind === 'angle')
    .map((entry, index) => {
...
export function validateRoiResultsBundleForSeries(bundle, series = state.manifest?.series?.[state.seriesIdx]) {
...
function rowPointGeometryFitsBundle(row = {}, series = {}) {
  if (!['polygon', 'ellipse', 'point', 'line', 'angle'].includes(row.kind)) return true;
  const points = cleanPoints(row.points);
...
export function importRoiResultsBundle(bundle, host = state) {
  const series = host?.manifest?.series?.[host.seriesIdx];
  const compatibility = validateRoiResultsBundleForSeries(bundle, series);
  ...
  for (const row of rows) {
    ...
    if (row.kind === 'angle') {
      const existing = angleRowsBySlice.get(sliceIdx) || angleEntriesForSlice(host, series.slug, sliceIdx).slice();
      if (row.roiObjectId && existing.some((item) => (
  ...
  for (const [sliceIdx, list] of angleRowsBySlice) setAngleEntriesForSlice(host, series.slug, sliceIdx, list);
...
export async function activateRoiResultRow(row, host = state) {
  if (!row) return false;
  const setSlice = () => {
