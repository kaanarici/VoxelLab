...
function measurementsHere() {
  ...
  return series ? measurementEntriesForSlice(state, series, state.sliceIdx) : [];
}
...
function measurementVisibleInCurrentScope(measurement) {
  const scope = currentMicroscopyScope();
  if (!scope || !measurement.microscopy) return true;
  return Number(measurement.microscopy.channelIndex || 0) === scope.channelIndex
    && Number(measurement.microscopy.timeIndex || 0) === scope.timeIndex;
}
