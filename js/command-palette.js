...
function render() {
  ...
  for (const sec of sections) {
    ...
    for (const cmd of filtered) {
      ...
      const sc = getShortcut(cmd.id)
        ? `<span class="cmdk-row-shortcut">${displayShortcutParts(getShortcut(cmd.id)).map(k => `<kbd>${escapeHtml(k)}</kbd>`).join('')}</span>`
        : '';
