...
function transformedValue(transform, key, fallback) {
  if (!transform || transform[key] == null) return fallback;
  return numericValue(transform[key], fallback);
}
...
export function extractEnhancedMultiFrameMetas(meta) {
  ...
  const sharedPixelValueTransform = seqItem(shared?.PixelValueTransformationSequence);
  ...
  const rootSlope = numericValue(meta?.RescaleSlope, 1);
  const rootIntercept = numericValue(meta?.RescaleIntercept, 0);
  const sharedSlope = transformedValue(sharedPixelValueTransform, 'RescaleSlope', rootSlope);
  const sharedIntercept = transformedValue(sharedPixelValueTransform, 'RescaleIntercept', rootIntercept);
  ...
  for (const frame of perFrame) {
    ...
    const pixelValueTransform = seqItem(frame?.PixelValueTransformationSequence);
    ...
    frameMetas.push({
      ...
      RescaleSlope: transformedValue(pixelValueTransform, 'RescaleSlope', sharedSlope),
      RescaleIntercept: transformedValue(pixelValueTransform, 'RescaleIntercept', sharedIntercept),
