...
export function referencedSeriesUID(meta, group = null) {
  const groupItem = contentItems(group).find((item) => itemMeaning(item) === 'Referenced Series UID');
  const groupUid = itemUid(groupItem);
  if (groupUid) return groupUid;
  return String(seqFirst(meta?.ReferencedSeriesSequence)?.SeriesInstanceUID || '').trim();
}
...
export function buildSRImport(meta, sourceSeries) {
  ...
  for (const group of groups) {
    ...
    const sourceUid = String(sourceSeries.sourceSeriesUID || sourceSeries.seriesUID || sourceSeries.seriesInstanceUID || '').trim();
    const referenceUid = referencedSeriesUID(meta, group);
    if (!reference
      || (referenceUid && referenceUid !== sourceUid)
      || (!referenceUid && reference.sourceSlug !== sourceSeries.slug)) {
