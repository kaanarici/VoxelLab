...
export function assertDicomFrameNumber(value, label = 'Frame number') {
  const frame = Number(value);
  if (!Number.isInteger(frame) || frame < 1) throw new Error(`Invalid ${label}: ${value}`);
  return frame;
}
...
function retryDelayMs(response, fallbackMs) {
  ...
  if (Number.isFinite(retryAt)) return Math.min(Math.max(0, retryAt - Date.now()), MAX_RETRY_AFTER_MS);
  return fallbackMs;
}

function addQueryParams(params, entries) {
...
function sortedQueryKey(query = {}) {
  ...
  addQueryParams(params, Object.keys(query)
    .sort()
...
function appendQuery(baseUrl, query = {}) {
  const params = new URLSearchParams();
  addQueryParams(params, Object.entries(query || {}));
...
function rememberSession(scopeKey, session) {
  const previousId = SESSION_ID_BY_SCOPE.get(scopeKey);
  if (previousId && previousId !== session.id) SESSION_BY_ID.delete(previousId);
...
function framePath(base, studyUID, seriesUID, instanceUID, frame = 1) {
  return `${trimTrailingSlashes(base)}/studies/${assertDicomUid(studyUID, 'Study UID')}/series/${assertDicomUid(seriesUID, 'Series UID')}/instances/${assertDicomUid(instanceUID, 'Instance UID')}/frames/${assertDicomFrameNumber(frame)}`;
}
...
class DicomwebTransportSession {
  ...
  async fetchFrameBytes({
    ...
    const frameNumber = assertDicomFrameNumber(frame);
    const key = frameKey(studyUID, seriesUID, instanceUID, frameNumber, accept);
    ...
    const pending = (async () => {
      const response = await this.fetchWithPolicy(
        framePath(this.wadoBase, studyUID, seriesUID, instanceUID, frameNumber),
        {
      ...
      if (!response?.ok) {
        throw new Error(`DICOMweb frame fetch failed (${instanceUID} frame ${frameNumber}): ${response?.status || 'unknown'}`);
      }
