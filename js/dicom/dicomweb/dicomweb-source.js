...
import {
  resolveDicomwebSession,
  dicomwebSessionStats,
  clearDicomwebSessions as clearTransportDicomwebSessions,
  assertDicomUid,
  assertDicomFrameNumber,
} from './session-transport.js';

const EXPLICIT_VR_LITTLE_ENDIAN = '1.2.840.10008.1.2.1';
const FRAME_ACCEPT = `application/octet-stream; transfer-syntax=${EXPLICIT_VR_LITTLE_ENDIAN}`;
const MAX_NORMALIZED_METADATA_CACHE = 96;
...
function optionalTagNumber(instance, tag) {
  const value = tagValue(instance, tag);
...
function normalizeFunctionalGroupItem(item) {
  ...
  const pixelValueTransforms = tagSequence(item, TAG.PixelValueTransformationSequence);
  ...
  if (pixelValueTransforms.length) {
    normalized.PixelValueTransformationSequence = pixelValueTransforms
      .map((seqItem) => {
        const transform = {};
        const slope = optionalTagNumber(seqItem, TAG.RescaleSlope);
        const intercept = optionalTagNumber(seqItem, TAG.RescaleIntercept);
        if (slope != null) transform.RescaleSlope = slope;
        if (intercept != null) transform.RescaleIntercept = intercept;
        return Object.keys(transform).length ? transform : null;
      })
      .filter(Boolean);
    if (!normalized.PixelValueTransformationSequence.length) delete normalized.PixelValueTransformationSequence;
  }
...
function rememberNormalizedMetadata(key, value) {
  if (_normalizedMetadataCache.has(key)) _normalizedMetadataCache.delete(key);
  _normalizedMetadataCache.set(key, value);
  while (_normalizedMetadataCache.size > MAX_NORMALIZED_METADATA_CACHE) {
    const oldest = _normalizedMetadataCache.keys().next().value;
    if (oldest == null) break;
    _normalizedMetadataCache.delete(oldest);
  }
}
...
async function mapConcurrent(items, limit, worker) {
  ...
  const width = Math.min(Number.isFinite(requested) && requested > 0 ? requested : 1, items.length);
...
export function clearDicomwebSessions() {
  clearTransportDicomwebSessions();
  _normalizedMetadataCache.clear();
}
...
export async function fetchSeriesMetadata(options) {
  ...
  if (options?.useCache !== false) rememberNormalizedMetadata(cacheKey, normalized);
  return normalized;
}
...
export async function fetchSeriesItems({
  ...
  const buffers = await mapConcurrent(requests, frameConcurrency, (request) => session.fetchFrameBytes({
    studyUID,
    seriesUID,
