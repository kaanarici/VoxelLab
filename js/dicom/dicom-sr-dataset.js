...
export function buildSRDataset(bundle) {
  ...
  const sourceSeriesUID = String(bundle.sourceSeriesUID || '').trim();
  ...
  const measurementGroups = measurements.map((m, i) => {
    ...
    if (m.kind === 'length') {
      const lengthMm = finiteNumber(m.length_mm);
      const lengthPx = finiteNumber(m.length_px);
      if (lengthMm != null) {
        container.ContentSequence.push({
      ...
      if (m.stats) {
        const areaMm2 = finiteNumber(m.stats.area_mm2);
        const mean = finiteNumber(m.stats.mean);
        const std = finiteNumber(m.stats.std);
        const adcMean = finiteNumber(m.stats.adc?.mean);
        if (areaMm2 != null) {
          container.ContentSequence.push({
            RelationshipType: 'CONTAINS',
            ValueType: 'NUM',
            ...
            MeasuredValueSequence: [{
              NumericValue: areaMm2.toFixed(2),
        ...
        if (adcMean != null) {
          container.ContentSequence.push({
            ...
            MeasuredValueSequence: [{
              NumericValue: (adcMean * 1e-3).toFixed(6),
    ...
    if (sourceSeriesUID) {
      container.ContentSequence.push({
        RelationshipType: 'HAS OBS CONTEXT',
        ValueType: 'UIDREF',
        ConceptNameCodeSequence: [codedValue('112002', 'DCM', 'Referenced Series UID')],
        UID: sourceSeriesUID,
      });
    }
