...
export function collectMeasurements(host) {
  ...
  const out = [];
  for (const entry of drawingEntriesForSeries(host, series)) {
    if (entry.kind === 'line') {
      const m = entry.data;
    ...
    } else if (entry.kind === 'polyline') {
      const r = entry.data;
      const lengthMm = finiteNumber(r.stats?.length_mm);
      const lengthPx = finiteNumber(r.stats?.length_px);
      const calibrated = spacing.known && lengthMm != null;
      if (!calibrated && lengthPx == null) continue;
      out.push({
        kind: 'length',
        slice: entry.sliceIdx,
        handles: r.pts,
        ...(calibrated ? { length_mm: lengthMm } : { length_px: lengthPx }),
      });
