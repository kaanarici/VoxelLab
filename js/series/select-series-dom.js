...
export function applySelectSeriesDom(i, series, v) {
  ...
  if (isMicroscopySeries(series)) {
    ...
    rows.splice(streamProvenanceText ? 5 : 4, 0, ['Axes', `Z ${series.microscopy?.sizeZ || series.slices} · C ${series.microscopy?.sizeC || 1} · T ${series.microscopy?.sizeT || 1}`]);
  ...
  meta.innerHTML = rows.map(metaRowHtml).join('');
  wireMetaCopy(meta);
