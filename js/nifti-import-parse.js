...
import { PAKO_ESM_URL } from './dependencies.js';
import { DEFAULT_IOP, normalize3 } from './geometry.js';
...
function niftiVoxelReader(view, offset, datatype, littleEndian) {
  const [bytes, getter] = NIFTI_TYPES[datatype] || [];
  return bytes ? (index) => view[getter](offset + index * bytes, littleEndian) : null;
}
...
export async function parseNIfTI(file, onProgress = () => {}) {
  ...
  const voxelAt = niftiVoxelReader(view, offset, datatype, littleEndian);
  ...
  for (let i = 0; i < totalVoxels; i += step) {
    samples.push(voxelAt(i) * scl_slope + scl_inter);
  }
  ...
  for (let z = 0; z < nz; z++) {
    ...
    for (let i = 0; i < nx * ny; i++) {
      const raw = voxelAt(base + i) * scl_slope + scl_inter;
