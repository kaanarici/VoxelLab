...
function computeStructureSignature(manifest, projects) {
  const folders = projects
    .map(p => `${p.id}:${p.collapsed ? 1 : 0}:${p.seriesSlugs.join('>')}`)
    .join('|');
  return [
    manifest.series.map(s => `${s.slug}\t${s.name || ''}\t${s.description || ''}`).join('>'),
    folders,
    getPinnedSlugs().join('>'),
    sidebar.currentSort || '',
    [...multiSel].sort().join('>'),
...
function applyActiveSelectionInPlace(list, pinnedList, activeSlug) {
  ...
  if (pinnedList) pinnedList.querySelectorAll('li[data-series-slug]').forEach(li => rows.push(li));

  let activeRow = null;
  for (const li of rows) {
    const isActive = li.dataset.seriesSlug === activeSlug;
    li.classList.toggle('active', isActive);
    if (isActive && !activeRow) activeRow = li;
  }
  if (activeSlug && !activeRow) return false;

  sidebar.flatOrder = rows.map(li => li.dataset.seriesSlug);
...
export async function renderProjectsSidebar(manifest, currentSeriesIdx) {
  ...
  if (
    ...
    && applyActiveSelectionInPlace(list, pinnedList, activeSlug)
  ) {
    return;
  }

  // Cold start: paint the flat list immediately so the sidebar isn't blank
  // while the tree is assembled. On warm rebuilds the existing rows stay until
  // they're replaced below, so no flash.
  ...
  if (pinnedList) {
    ...
    for (const s of pinnedSeries) {
      ...
      li.innerHTML = `
        ...
        <div class="sname"><span class="sname-text">${escapeHtml(s.name || '')}</span>${seriesCloudMarkHtml(s)}</div>
      `;
...
function seriesCloudMarkHtml(s = {}) {
  ...
  const resultStatus = String(cloudAction?.resultStatus || '').trim();
  const statusLabel = resultStatus && resultStatus.toLowerCase() !== 'complete' ? ` · status ${resultStatus.replaceAll('_', ' ')}` : '';
  const title = `${cloudLabel}${cloudJobId ? ` · job ${cloudJobId}` : ''}${statusLabel}`;
  return `<span class="series-cloud-mark" title="${escapeHtml(title)}" aria-label="${escapeHtml(title)}">
        <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
          <path d="M17.5 18H8a4 4 0 0 1-.8-7.9 5.5 5.5 0 0 1 10.5 1.5H18a3.2 3.2 0 0 1-.5 6.4Z"/>
        </svg>
      </span>`;
}
