...
function stepsField(payload, envelope) {
  if (!Object.hasOwn(payload, 'steps')) return null;
...
const ASK_ACTION_IDS = new Set(['open-cloud-workflow', 'open-cloud-results', 'open-registration-compare']);

function actionsField(payload, envelope) {
  if (!Object.hasOwn(payload, 'actions')) return null;
  const value = payload.actions;
  if (!Array.isArray(value)) throw new EnvelopeValidationError(envelope, 'actions_not_array');
  ...
  return value.map((item) => {
    ...
    const id = stringField(item, 'id', envelope);
    if (!ASK_ACTION_IDS.has(id)) throw new EnvelopeValidationError(envelope, `action_unknown:${id}`);
    if (seen.has(id)) throw new EnvelopeValidationError(envelope, `action_duplicate:${id}`);
    seen.add(id);
    return {
      id,
      label: stringField(item, 'label', envelope),
      detail: stringField(item, 'detail', envelope, { allowEmpty: true }),
    };
  });
}
...
function normalizeAskFields(payload, envelope) {
  ...
  const steps = stepsField(payload, envelope);
  if (steps) normalized.steps = steps;
  const actions = actionsField(payload, envelope);
...
export function normalizeAskEntry(value) {
  ...
  unexpected(payload, new Set(['key', 'slice', 'x', 'y', 'question', 'answer', 'crop', 'region', 'contextFingerprint', 'steps', 'actions']), envelope);
...
export function normalizeAskResult(value) {
  ...
  const payload = objectValue(value, envelope);
  unexpected(payload, new Set(['cached', 'key', 'slice', 'x', 'y', 'question', 'answer', 'crop', 'region', 'contextFingerprint', 'steps', 'actions']), envelope);
