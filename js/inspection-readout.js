import { TISSUE_NAMES, CT_HU_LO, CT_HU_RANGE } from './core/constants.js';
...
export function renderInspectionReadout(info, { coordLabel = 'px', includeSlice = false } = {}) {
  ...
  if (Number.isFinite(info.ctHu)) {
    const pfx = info.ctHuClipped === 'high' ? '≥' : info.ctHuClipped === 'low' ? '≤' : '';
    const sfx = info.ctHuClipped ? ' (clipped)' : '';
    html += `<div><span class="hv-label">HU</span>${pfx}${info.ctHu}${sfx}</div>`;
  }
