...
export function desktopConversionDialogText(started = [], skipped = []) {
  ...
  if (skipped.length) {
    const firstReason = skipped.find(record => record?.conversionReason)?.conversionReason || '';
    const env = skipped.find(record => record?.converterEnv)?.converterEnv || 'the converter environment variable';
    if (!firstReason) {
      messages.push(
        `Configure an OME-TIFF converter before opening ${plural(skipped.length, 'converter-backed file')}${skippedNames ? `: ${skippedNames}` : ''}.`,
      );
    } else {
      const diagnostic = firstReason === 'converter_path_missing'
        ? `${env} points to a missing executable.`
        : firstReason === 'converter_path_not_executable'
          ? `${env} must point to an executable file.`
          : firstReason === 'converter_path_not_absolute'
            ? `${env} must be an absolute executable path.`
            : `Configure ${env} with an OME-TIFF converter executable.`;
      messages.push(
        `${diagnostic} Cannot open ${plural(skipped.length, 'converter-backed file')}${skippedNames ? `: ${skippedNames}` : ''} until it is configured.`,
      );
    }
  }
