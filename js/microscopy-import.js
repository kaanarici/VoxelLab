import { DEFAULT_IOP } from './geometry.js';
import {
...
import { lengthUnitToMm, normalizeLengthUnit } from './physical-units.js';
...
export function parseImageJDescriptionMetadata(description = '', pageCount = 1, page = null) {
  ...
  const hasImageJUnit = String(lines.unit || '').trim().length > 0;
  ...
  return {
    ...
    physicalUnit: normalizeLengthUnit(lines.unit || 'µm'),
    channelNames: [],
  };
}
