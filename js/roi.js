...
import { formatAreaFromMm2 } from './physical-units.js';
import {
...
function computeStats(pts, shape) {
  ...
  if (shape === 'point') {
    ...
    return { pixels: n, mean, std: Math.sqrt(variance), min, max, count: n };
  }
...
export function countROIs() {
  ...
  return drawingEntriesForSeries(host, slug)
    .filter((entry) => entry.kind === 'ellipse' || entry.kind === 'polygon' || entry.kind === 'point')
    .length;
}
