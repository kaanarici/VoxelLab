...
function pixelTypeForPage(page = {}) {
  ...
  if (sampleFormat === 1 && bits === 32) return 'uint32';
  if (sampleFormat === 2 && bits === 8) return 'int8';
  if (sampleFormat === 2 && bits === 16) return 'int16';
  if (sampleFormat === 2 && bits === 32) return 'int32';
  if (sampleFormat === 3 && bits === 32) return 'float32';
  return `sampleFormat${sampleFormat || 'unknown'}:${bits || 'unknown'}bit`;
}

export function rawMicroscopyValueSource(series = {}) {
  return /^(?:u?int|float)32$/.test(String(series?.microscopyDataset?.pixel?.type || ''))
    ? 'raw_scalar'
    : 'raw_16bit';
}
...
function warningsForAxes({ xScale, yScale, zScale, xUnitKnown, yUnitKnown, zUnitKnown, sizeZ }) {
  const warnings = [];
  const zRequired = Number(sizeZ) > 1;
  if (!(xScale > 0 && xUnitKnown) || !(yScale > 0 && yUnitKnown)) warnings.push('missing_xy_physical_size');
  if (zRequired && !(zScale > 0 && zUnitKnown)) warnings.push('missing_z_physical_size');
  ...
  if (zRequired && zScale > 0 && !zUnitKnown) warnings.push('unsupported_z_physical_unit');
  return warnings;
}
...
export function channelByIndex(dataset, index) {
  ...
  if (!cached || cached.channels !== channels || cached.length !== channels.length) {
    ...
    for (const channel of channels) {
      const key = Number(channel?.index);
      if (!map.has(key)) map.set(key, channel);
    }
    cached = { channels, length: channels.length, map };
    CHANNEL_BY_INDEX.set(dataset, cached);
  }
  return cached.map.get(Number(index)) || null;
}
...
export function buildMicroscopyDataset({
  ...
  return {
    ...
    source: {
      ...
      signatures: Array.isArray(metadata.sourceSignatures)
        ? metadata.sourceSignatures.map(signature => ({
          name: String(signature?.name || ''),
          relativePath: String(signature?.relativePath || ''),
          byteLength: Number(signature?.byteLength || 0),
          sampleFingerprint: String(signature?.sampleFingerprint || ''),
        }))
        : [],
