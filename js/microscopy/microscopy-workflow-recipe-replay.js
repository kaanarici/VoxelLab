...
function trustedMeasurementsAvailable(series, host = state, recipe = null) {
  ...
  if (rows.some((entry) => entry?.kind === 'ellipse' || entry?.kind === 'polygon' || entry?.kind === 'line')) return true;
  const bundle = embeddedRoiResultsBundle(recipe);
  return !!bundle?.calibration?.xyKnown
    && embeddedMeasurementRows(recipe).some((row) => row?.kind === 'ellipse' || row?.kind === 'polygon' || row?.kind === 'line');
}
...
export function validateAnalysisOps(recipe, series, dims = null) {
  ...
  for (const op of ops) {
    if (!op || op.op !== 'analyze-particles') {
      return { ok: false, code: 'unknown_analysis_op', message: 'Recipe contains an unsupported analysis operation.' };
    }
    const { c, z, t } = op.inputs || {};
    if (!(finiteInteger(c, -1) >= 0 && finiteInteger(c, -1) < sizeC)
      || !(finiteInteger(t, -1) >= 0 && finiteInteger(t, -1) < sizeT)
    ...
    if (!Number.isFinite(Number(op.params?.threshold?.resolvedValue))) {
...
function applyEmbeddedAngleMeasurements(recipe, series, host = state) {
  ...
  for (const [sliceIndex, sliceRows] of bySlice.entries()) {
    const live = angleEntriesForSlice(host, series.slug, sliceIndex).map((entry) => ({ ...entry }));
    const seen = new Set(live.map((entry, index) => entry.recipeAngleObjectId || angleObjectId(series, sliceIndex, entry, index)));
    ...
    setAngleEntriesForSlice(host, series.slug, sliceIndex, next);
  }
  return { ok: true, code: '', message: '' };
}
...
export function applyMicroscopyWorkflowRecipe(recipe, host = state) {
  ...
  const previous = {
    ...
    analysisLog: Array.isArray(host?._microscopyAnalysisLog?.[series.slug])
      ? host._microscopyAnalysisLog[series.slug].slice()
      : [],
  };
  ...
  const rollback = () => {
    ...
    for (const { sliceIdx, ids } of createdAnalysis) {
      const kept = roiEntriesForSlice(series.slug, sliceIdx).filter((entry) => !ids.has(entry.importedObjectId));
      setRoiEntriesForSlice(series.slug, sliceIdx, kept);
    }
    if (host._microscopyAnalysisLog) host._microscopyAnalysisLog[series.slug] = previous.analysisLog;
  };
  try {
    ...
    for (const descriptor of analysisOps) {
      ...
      const liveIds = new Set(roiResultRows(host, series).map((row) => row.objectId));
      if (expected.size > 0 && [...expected].every((id) => liveIds.has(id))) continue; // idempotent re-apply
    ...
    if (analysisOps.length && host._microscopyAnalysisLog) {
      host._microscopyAnalysisLog[series.slug] = analysisOps.slice();
    }
    return { ok: true, code: '', message: '' };
  } catch (error) {
    rollback();
