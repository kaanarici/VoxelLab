...
export function drawMicroscopyChannelComposite(ctx, width, height, {
  ...
  if (!_compositeImage || _compositeImage.width !== width || _compositeImage.height !== height) {
    _compositeImage = ctx.createImageData(width, height);
  }
...
export function drawMicroscopyChannelSplit(ctx, width, height, {
  ...
  for (let c = 0; c < active.length; c += 1) {
    if (!_splitImage || _splitImage.width !== width || _splitImage.height !== height) {
      _splitImage = ctx.createImageData(width, height);
    }
    const image = _splitImage;
    const out = image.data;
