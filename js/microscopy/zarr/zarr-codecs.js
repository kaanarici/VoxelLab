...
export async function decodeZarrChunk(bytes, { compressor = null, filters = null, dtype, expectedBytes } = {}) {
  if (hasFilters(filters)) throw unsupported('filters');
  validateDtype(dtype);
  ...
  try {
    if (id === 'raw') return validateDecodedLength(new Uint8Array(src), length);
    if (id === 'blosc') {
      validateBloscRequest(compressor);
      return validateDecodedLength(await bloscDecompress(src), length);
    }
    if (id === 'zlib' || id === 'gzip') return validateDecodedLength(await inflateWithPako(src), length);
    if (id === 'zstd') return validateDecodedLength(await decompressWithFzstd(src), length);
    throw unsupported(`compressor.id '${id || 'unknown'}'`);
  } catch (error) {
...
export function describeZarrCodec(compressor, filters) {
  const filterSuffix = hasFilters(filters) ? ' + filters' : '';
  const id = compressorId(compressor);
