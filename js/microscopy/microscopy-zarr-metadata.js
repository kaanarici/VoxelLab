...
const ISSUE_LABELS = new Map([
  ['axes_channel_or_custom_order_invalid', 'Channel/custom axes must appear before spatial axes'],
  ['axes_dimension_count_mismatch', 'Array dimensions do not match declared axes'],
  ['axes_length_out_of_range', 'OME-Zarr axes must contain 2-5 dimensions'],
  ['axes_space_count_invalid', 'OME-Zarr metadata must define two or three spatial axes'],
  ['axes_time_order_invalid', 'Time axis must appear before channel and spatial axes'],
  ['datasets_missing', 'OME-Zarr metadata has no datasets'],
  ['multiscales_missing', 'OME-Zarr metadata has no multiscales entry'],
]);
...
export function omeZarrMetadataIssueLabels(codes = []) {
  return Array.from(codes || [])
    .map((code) => ISSUE_LABELS.get(String(code || '')) || readableIssueCode(code))
    .filter(Boolean);
}
