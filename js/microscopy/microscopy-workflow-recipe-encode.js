...
export function measurementRows(series, host = state) {
  return drawingEntriesForSeries(host, series)
    .filter((entry) => entry?.kind === 'ellipse' || entry?.kind === 'polygon' || entry?.kind === 'polyline' || entry?.kind === 'point' || entry?.kind === 'line');
}

function angleMeasurementRows(series, host = state) {
  return drawingEntriesForSeries(host, series).filter((entry) => entry?.kind === 'angle');
}
...
export function captureMicroscopyWorkflowRecipe(host = state) {
  ...
  const analysisOps = Array.isArray(host?._microscopyAnalysisLog?.[analysisIdentity])
    ? host._microscopyAnalysisLog[analysisIdentity]
    : [];
  return {
