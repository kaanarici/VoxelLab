...
const MAX_MISSING_PLANE_INDICES = 128;
...
function buildWarnings(groupId, files) {
  ...
  if (indices.length > 1) {
    ...
    for (let index = 1; index < indices.length; index += 1) {
      const start = indices[index - 1] + 1;
      const end = indices[index];
      if (start >= end) continue;
      missingCount += end - start;
      for (let value = start; value < end && missing.length < MAX_MISSING_PLANE_INDICES; value += 1) {
        missing.push(value);
      }
    }
    if (missingCount) {
