...
let _cloudProcessingEnabled = true;
...
const IMPORTABLE_TERMINAL_STATUSES = new Set(['complete', 'partial']);
const FAILED_TERMINAL_STATUSES = new Set(['error', 'failed', 'canceled', 'cancelled', 'dispatch_unknown']);
...
function cloudStatusProgressDetail(status = {}) {
  const raw = status.progress || status.message || status.stage || status.status || '';
  const value = String(raw || '').trim();
  if (!value || value === 'running') return 'running...';
  return value;
}

function cloudStatusValue(status = {}) {
  return String(status?.status || '').trim().toLowerCase();
}

function cloudTerminalFailureMessage(status = {}) {
  const statusValue = cloudStatusValue(status);
  const detail = String(status?.error || status?.message || status?.detail || status?.reason || '').trim();
  if (statusValue === 'canceled' || statusValue === 'cancelled') {
    return detail ? `Cloud job canceled: ${detail}` : 'Cloud job canceled';
  }
  if (statusValue === 'failed') {
    return detail ? `Cloud job failed: ${detail}` : 'Cloud job failed';
  }
  return detail || 'Processing failed';
}
...
function stoppedCloudError(signal) {
  const error = terminalCloudError(cloudStopMessage(signal));
  error.cloudStopped = true;
  return error;
}

function normalizeAbortError(error, signal) {
  if (error?.cloudStopped) return error;
  if (signal?.aborted || error?.name === 'AbortError') return stoppedCloudError(signal);
  return error;
}

function throwIfStopped(signal) {
  if (signal?.aborted) throw stoppedCloudError(signal);
}
...
function abortableDelay(ms, signal) {
  ...
  return new Promise((resolve, reject) => {
    ...
    function onAbort() {
      clearTimeout(timer);
      reject(stoppedCloudError(signal));
    }
...
async function cloudFetch(url, options = {}, signal) {
  throwIfStopped(signal);
  try {
...
export function initCloud(webhookBaseUrl, r2PublicUrl = '', options = {}) {
  _cloudProcessingEnabled = options.enabled !== false && options.cloudProcessing !== false;
...
export function isCloudAvailable() {
  return cloudRuntimeStatus().available;
}

export function cloudRuntimeStatus() {
  if (_cloudProcessingEnabled === false) {
    return {
      available: false,
      code: 'disabled',
      message: 'Cloud GPU processing is disabled.',
    };
  }
  if (!_webhookBase) {
    return {
      available: false,
      code: 'setup-required',
      message: 'Cloud GPU processing needs Modal webhook settings.',
    };
  }
  if (!(trustedUploadOrigins().length || _r2PublicBase)) {
    return {
      available: false,
      code: 'storage-required',
      message: 'Cloud GPU processing needs an R2 public URL or trusted upload origin.',
    };
  }
  return {
    available: true,
    code: 'ready',
    message: 'Cloud GPU processing is ready.',
  };
}
...
async function uploadFiles(items, jobId, onProgress, signal) {
  ...
  for (const urlBatch of chunks(items, UPLOAD_URL_BATCH_SIZE)) {
    ...
    const urlResp = await cloudFetch(cloudEndpoint('get_upload_urls'), {
      method: 'POST',
    ...
    const uploadTasks = urlBatch.map(item => async () => {
      ...
      await uploadFile(url, item.file, signal);
      uploaded++;
...
export async function uploadAndProcess(files, onProgress = () => {}, processing = {}, options = {}) {
  ...
  const progressMeta = { jobId, processing, totalUploadBytes, abortable: !!signal };
  const progress = (stage, detail) => onProgress(stage, detail, progressMeta);
  ...
  throwIfStopped(signal);
  progress('preparing', `${uploadItems.length} files`);
  await uploadFiles(uploadItems, jobId, progress, signal);

  progress('processing', 'starting GPU pipeline...');
  const startResp = await cloudFetch(cloudEndpoint('start_processing'), {
    method: 'POST',
    headers: cloudHeaders({ 'Content-Type': 'application/json' }),
  ...
  while (Date.now() - start < maxWait) {
    ...
    await abortableDelay(cloudPollDelay(elapsedMs), signal);
    if (Date.now() - lastProgressAt >= 1000) {
      progress('processing', `running... (${Math.round((Date.now() - start) / 1000)}s)`);
      lastProgressAt = Date.now();
    }
    let status;
    try {
      const statusResp = await cloudFetch(cloudEndpoint('check_status'), {
        method: 'POST',
        headers: cloudHeaders({ 'Content-Type': 'application/json' }),
        body: JSON.stringify({ job_id: jobId }),
      }, signal);
      if (!statusResp.ok) {
        lastFailureDetail = `HTTP ${statusResp.status}`;
        if (++consecutiveFailures >= MAX_CONSECUTIVE_STATUS_FAILURES) {
          throw terminalCloudError(`Cloud status check kept failing (${lastFailureDetail})`);
        }
        continue;
      }
      status = await statusResp.json();
    } catch (e) {
      if (e?.cloudStopped) throw e;
    ...
    const statusValue = cloudStatusValue(status);
    if (IMPORTABLE_TERMINAL_STATUSES.has(statusValue)) {
      beginPerfTrace(`cloud-${statusValue}-to-paint`, { jobId });
      ...
      const { slug } = result;
      progress(statusValue, slug);
      return result;
    }
    if (FAILED_TERMINAL_STATUSES.has(statusValue)) {
      throw terminalCloudError(cloudTerminalFailureMessage(status));
    }
    progress('processing', cloudStatusProgressDetail(status));
  }
  const detail = lastFailureDetail ? ` (last status check: ${lastFailureDetail})` : '';
  throw new Error(`Processing timed out after 20 minutes${detail}`);
}
