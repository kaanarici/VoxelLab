...
def normalize_source_manifest(payload: Any) -> dict[str, Any] | None:
    ...
    if normalized.get("sourceKind") == "registration" and isinstance(normalized.get("registration"), dict):
        registration = dict(normalized["registration"])
        transform = str(registration.get("transform", registration.get("transformType", "rigid")) or "rigid")
...
def registration_manifest_errors(payload: Any, series_uids: set[str] | list[str] | tuple[str, ...]) -> list[str]:
    ...
    if not payload.get("sourceRecordVersion"):
        errors.append(f"source manifest.sourceRecordVersion: expected one of {sorted(SOURCE_RECORD_VERSIONS)}")
    if payload.get("sourceKind") != "registration":
        return ["source manifest.sourceKind: expected registration"]

    registration = payload.get("registration")
    if not isinstance(registration, dict):
        return errors + ["source manifest.registration: expected object"]

    available = {str(uid) for uid in series_uids if str(uid)}
    fixed = str(registration.get("fixedSeriesUID", "") or "")
    moving = str(registration.get("movingSeriesUID", "") or "")
    if not fixed:
        errors.append("source manifest.registration.fixedSeriesUID: expected non-empty string")
    elif fixed not in available:
        errors.append("source manifest.registration.fixedSeriesUID: expected one selected DICOM series UID")
    if not moving:
        errors.append("source manifest.registration.movingSeriesUID: expected non-empty string")
    elif moving not in available:
        errors.append("source manifest.registration.movingSeriesUID: expected one selected DICOM series UID")
    if fixed and moving and fixed == moving:
        errors.append("source manifest.registration.movingSeriesUID: expected a different series than fixedSeriesUID")

    transform = str(registration.get("transform", "rigid") or "rigid")
    if transform not in REGISTRATION_TRANSFORMS:
...
def registration_summary(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "status": "configured",
        "transform": str(registration.get("transform", "rigid") or "rigid"),
        "fixedSeriesUID": str(registration.get("fixedSeriesUID", "") or ""),
        "movingSeriesUID": str(registration.get("movingSeriesUID", "") or ""),
        "source": "external-json",
        "sourceRecordVersion": int(payload.get("sourceRecordVersion", 1) or 1),
    }
