...
from json_store import atomic_write_json, file_lock, update_json
...
def _validate_analysis_key(analysis_key: str | None) -> str | None:
    if analysis_key is None:
        return None
    if not isinstance(analysis_key, str) or not ANALYSIS_KEY_RE.fullmatch(analysis_key):
...
def _process_locked(
    ...
    if out_path.exists():
        try:
            ...
            if analysis_key is not None:
                raise ValueError(f"keyed analysis result is invalid: {out_path.name}") from exc
            persisted = {}
        if analysis_key is not None and (
            persisted.get("analysisKey") != analysis_key or persisted.get("slug") != slug
        ):
            raise ValueError("analysis key is already bound to another result identity")
        if not force:
            existing = persisted
    ...
    for i in need:
        ...
        if not png.exists():
            raise RuntimeError(f"slice {i} image is missing: {png}")
        try:
...
def process(
    ...
    slug = _validate_slug(slug)
    analysis_key = _validate_analysis_key(analysis_key)
    lock_name = analysis_key.replace(':', '-') if analysis_key is not None else slug
    operation_lock = DATA / f".{lock_name}_analysis.operation.lock"
    with file_lock(operation_lock, timeout=None):
...
def update_manifest(slugs):
    ...
    def mark_analyzed(manifest: dict) -> dict:
        for series in manifest["series"]:
            if series["slug"] in wanted:
                series["hasAnalysis"] = True
...
def main() -> bool:
    ...
    _ = ap.add_argument("--analysis-key", help="opaque v2 source-series identity for a local viewer job")
    args = ap.parse_args()
    ...
    if args.analysis_key is not None and len(requested) != 1:
        print("ERROR: --analysis-key requires exactly one series slug", file=sys.stderr, flush=True)
        return False
    ...
    for slug in requested:
        ...
        try:
            ...
            print(f"{slug}: analysis failed: {e}", file=sys.stderr, flush=True)
            ok = False
            continue
        if completed and args.analysis_key is None:
