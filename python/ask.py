...
def _stream_ai(prompt: str, system: str, schema: dict, *, model, provider, images, add_dirs, timeout, on_event: SliceEvent) -> dict:
    ...
    for event in stream_structured(prompt=prompt, system=system, schema=schema, model=model, provider=provider, images=images, add_dirs=add_dirs, timeout=timeout):
...
def _call_ai(
    ...
    if on_event is not None:
        return _stream_ai(prompt, system, schema, model=model, provider=provider, images=images, add_dirs=add_dirs, timeout=timeout, on_event=on_event)
...
def _save_asks(slug: str, data: dict) -> None:
    ...
    if not isinstance(entries, list):
        raise EnvelopeValidationError("ask-sidecar", "entries_not_array")
    sidecar = {"slug": slug, "entries": [normalize_ask_entry(entry) for entry in entries]}
    _ = _ask_cache_path(slug).write_text(json.dumps(sidecar, indent=2))
