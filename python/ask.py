...
def _cached_ask(data: dict, key: str, context_fingerprint: str | None) -> dict | None:
    for entry in data.get("entries", []):
        ...
        if entry.get("contextFingerprint") != context_fingerprint:
            continue
        return normalize_ask_result({"cached": True, **normalize_ask_entry(entry)})
    return None
...
def ask(
    ...
    *,
    ...
    if region is None and x is None and y is None:
        return _ask_study(slug, meta, slice_idx, question, model=model, provider=provider, on_event=on_event)
