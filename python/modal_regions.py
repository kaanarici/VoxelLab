...
def region_volume_stats(slug: str, label_vol, legend: dict[int, str], pixel_spacing: list[float], slice_thickness: float) -> dict[str, Any]:
    ...
    region_volumes = []
    for rid in sorted(legend):
        count = int((label_vol == rid).sum())
        if count <= 0:
            continue
        region_volumes.append({
            "id": str(rid),
            "label": str(legend[rid]),
            "name": humanize_region_name(str(legend[rid])),
            "voxels": count,
            "volumeMl": round(count * voxel_ml, 3),
            "source": "TotalSegmentator",
        })
    return {
        "slug": slug,
        "statsKind": "region-volumes",
        "source": "modal:totalsegmentator",
        "sourceRegions": f"data/{slug}_regions.json",
        "voxelVolumeMl": voxel_ml,
        "unit": "mL",
...
def write_region_stats(slug: str, label_vol, legend: dict[int, str], out_root: Path, pixel_spacing: list[float], slice_thickness: float) -> Path:
    ...
    _ = stats_path.write_text(json.dumps(region_volume_stats(slug, label_vol, legend, pixel_spacing, slice_thickness), indent=2))
    return stats_path
