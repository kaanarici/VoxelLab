...
def _actions(payload: dict[str, Any], envelope: str) -> list[dict[str, str]] | None:
    ...
    if not isinstance(value, list):
        raise EnvelopeValidationError(envelope, "actions_not_array")
    if len(value) > 4:
        raise EnvelopeValidationError(envelope, "actions_too_many")
    seen: set[str] = set()
    actions: list[dict[str, str]] = []
    for item in value:
        if not isinstance(item, dict):
            raise EnvelopeValidationError(envelope, "action_not_object")
        _unexpected(item, {"id", "label", "detail"}, envelope)
        action_id = _string(item, "id", envelope)
        if action_id not in ASK_ACTION_IDS:
            raise EnvelopeValidationError(envelope, f"action_unknown:{action_id}")
        if action_id in seen:
            raise EnvelopeValidationError(envelope, f"action_duplicate:{action_id}")
        seen.add(action_id)
        actions.append({
            "id": action_id,
            "label": _string(item, "label", envelope),
            "detail": _string(item, "detail", envelope, allow_empty=True),
        })
    return actions
...
def _normalize_ask_fields(payload: dict[str, Any], envelope: str) -> dict[str, Any]:
    ...
    actions = _actions(payload, envelope)
...
def normalize_ask_entry(value: Any) -> dict[str, Any]:
    ...
    _unexpected(
        payload,
        {"key", "slice", "x", "y", "question", "answer", "crop", "region", "contextFingerprint", "steps", "actions"},
        envelope,
    )
...
def normalize_ask_result(value: Any) -> dict[str, Any]:
    ...
    _unexpected(
        payload,
        {"cached", "key", "slice", "x", "y", "question", "answer", "crop", "region", "contextFingerprint", "steps", "actions"},
        envelope,
    )
