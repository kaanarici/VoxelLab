...
def prune_terminal_statuses(
    running: dict,
    *,
    max_terminal: int = MAX_TERMINAL_ANALYSIS_STATUS_ENTRIES,
    preserve_keys=(),
) -> int:
    keep = set(preserve_keys)
    terminal = [
        (key, entry)
        for key, entry in running.items()
        if entry.get("status", "running") != "running" and key not in keep
    ]
    excess = max(0, len(terminal) + sum(
        1 for key in keep
        if key in running and running[key].get("status", "running") != "running"
    ) - max(0, max_terminal))
    if not excess:
        return 0
    terminal.sort(key=lambda item: float(item[1].get("lastUpdated") or 0))
    ...
    try:
        for raw in proc.stdout:
            ...
            if line.startswith("ERROR:") or " ERROR:" in line or "analysis failed:" in line:
                last_error = line
            with lock:
                entry = running.get(analysis_key)
...
def start_analysis(
    ...
    *,
    ...
    analysis_key = normalize_analysis_key(analysis_key)
    if not analysis_key:
        return 400, "invalid analysis key"
    with lock:
        existing = running.get(analysis_key)
        if existing and existing.get("slug") != slug:
            return 409, "analysis key is already bound to another slug"
        if existing and existing.get("status") == "running":
            return 409, "already running for this source series"
        active_jobs = sum(1 for entry in running.values() if entry.get("status", "running") == "running")
        if active_jobs >= MAX_CONCURRENT_ANALYSIS_JOBS:
            return 429, "too many analysis jobs are already running"
        _ = prune_terminal_statuses(running, preserve_keys={analysis_key})
        running[analysis_key] = analysis_status_entry(
    ...
    try:
        ...
        with lock:
            running.pop(analysis_key, None)
        return 500, f"failed to start analysis: {exc}"
    with lock:
        running[analysis_key] = analysis_status_entry(
            time_module, proc=proc, status="running", last="starting...", analysis_key=analysis_key, slug=slug,
        )
...
def status_payload(running: dict, lock, analysis_key: str | None = None) -> dict:
    ...
    with lock:
        _ = prune_terminal_statuses(running, preserve_keys={normalized} if normalized else ())
        entries = running.items() if normalized is None else (
            [(normalized, running[normalized])] if normalized in running else []
        )
        return {
            analysis_key: {
...
def handle_ai_post(
    ...
    if parsed.path == "/api/analyze":
        ...
        analysis_key = normalize_analysis_key((qs.get("analysisKey") or [""])[0])
        ...
        if not slug:
            ...
            return
        if not analysis_key:
            handler._json(400, {"error": "invalid analysis key"})
            return
        slices = None
        if raw_slices:
            ...
            if error:
                ...
                return
        code, msg = start_analysis(slug, analysis_key=analysis_key, force=force, slices=slices)
        handler._json(code, {
            "message": msg,
            "slug": slug,
            "analysisKey": analysis_key,
...
def handle_ai_get(handler, parsed, data_dir: Path, has_local_api_token, status_payload) -> bool:
    if parsed.path == "/api/analyze/result":
        raw_key = (parse_qs(parsed.query, keep_blank_values=True).get("analysisKey") or [None])[0]
        analysis_key = normalize_analysis_key(raw_key) if raw_key is not None else None
        if not analysis_key:
            handler._json(400, {"error": "invalid analysis key"})
            return True
        path = data_dir / analysis_result_filename(analysis_key)
        if not path.exists():
            handler._json(200, {})
            return True
        try:
            ...
            handler._json(409, {"error": "analysis result is invalid", "resultError": str(exc)})
            return True
    ...
    if parsed.path == "/api/analyze/status":
        ...
        raw_key = (parse_qs(parsed.query, keep_blank_values=True).get("analysisKey") or [None])[0]
        analysis_key = normalize_analysis_key(raw_key) if raw_key is not None else None
        if raw_key is not None and not analysis_key:
            handler._json(400, {"error": "invalid analysis key"})
            return True
