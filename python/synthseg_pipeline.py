...
def _remap_synthseg_labels(seg_dhw: np.ndarray) -> np.ndarray:
...
def _synthseg_region_payload(label_vol: np.ndarray, px_x: float,
                             px_y: float, slice_mm: float) -> dict:
...
def read_synthseg_output(seg_nii_path: Path, ref_nii_path: Path,
    ...
    return _remap_synthseg_labels(seg_dhw)
...
def write_outputs(slug: str, label_vol: np.ndarray, px_x: float,
    ...
    payload = _synthseg_region_payload(label_vol, px_x, px_y, slice_mm)
    regions = payload["regions"]

    stats_path = DATA / f"{slug}_regions.json"
    _ = stats_path.write_text(json.dumps(payload, indent=2))
...
def main() -> bool:
    ...
    updates = {slug: {"hasRegions": True, "anatomySource": "synthseg"} for slug in successes}
