...
PROCESSING_MODE_DEFAULT_INPUT_KIND = {
    ...
    "rigid_registration": "dicom_registration_pair",
}
...
def validate_input_kind(kind: object, processing_mode: str = "standard") -> str:
    ...
    if not isinstance(kind, str) or kind not in INPUT_KINDS:
        return ""
    return kind if kind == default else ""


def input_kind_error(kind: object, processing_mode: str = "standard") -> str:
    if validate_input_kind(kind, processing_mode):
        return ""
    expected = default_input_kind(processing_mode)
    if expected and isinstance(kind, str) and kind in INPUT_KINDS:
        return f"{processing_mode} requires {expected} input_kind"
    return "invalid input_kind"
