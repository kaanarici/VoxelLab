...
def _orthonormal_iop(iop: list[float], tolerance: float = 1e-3) -> tuple[list[float], list[float], str]:
    ...
    row_raw = iop[:3]
    col_raw = iop[3:6]
    row_norm = norm3(row_raw)
    col_norm = norm3(col_raw)
    ...
    row = normalize3(row_raw)
    col = normalize3(col_raw)
...
def _volume_stack_geometry_error(slices: list) -> str:
    ...
    base_iop = float_list(getattr(slices[0], "ImageOrientationPatient", []), 6)
    base_row, base_col, error = _orthonormal_iop(base_iop)
    if error:
        return error
    normal = normalize3(cross3(base_row, base_col))
    if not normal:
        return "MPR volume processing requires nonzero slice normal and slice span"

    base_spacing = _positive_pixel_spacing(slices[0])
    if len(base_spacing) < 2:
        return "MPR volume processing requires positive DICOM PixelSpacing"

    positions: list[dict[str, list[float] | float]] = []
    seen_projections: set[float] = set()
    for ds in slices:
        frame_uid = str(getattr(ds, "FrameOfReferenceUID", "") or "").strip()
        if frame_uid != base_frame_uid:
            return "MPR volume processing requires consistent FrameOfReferenceUID"
        ...
        if len(spacing) < 2:
            return "MPR volume processing requires positive DICOM PixelSpacing"
        if not (_same_number(spacing[0], base_spacing[0]) and _same_number(spacing[1], base_spacing[1])):
            return "MPR volume processing requires consistent DICOM PixelSpacing"

        ipp = float_list(getattr(ds, "ImagePositionPatient", []), 3)
        if len(ipp) < 3:
            return "MPR volume processing requires ImagePositionPatient on every slice"
        projection = dot3(ipp, normal)
        if positions:
            first_ipp = positions[0]["ipp"]
            assert isinstance(first_ipp, list)
            delta = [ipp[i] - first_ipp[i] for i in range(3)]
            axial = dot3(delta, normal)
            residual = [delta[i] - normal[i] * axial for i in range(3)]
            if norm3(residual) > 0.1:
                return "MPR volume processing requires slice positions aligned with the orientation normal"
        projection_key = round(projection, 3)
        if projection_key in seen_projections:
            return "MPR volume processing requires distinct slice positions"
        seen_projections.add(projection_key)
        positions.append({"ipp": ipp, "projection": projection})

    projections = [float(item["projection"]) for item in positions]
    if max(projections) - min(projections) < 0.01:
        return "MPR volume processing requires nonzero slice normal and slice span"
    rounded = {round(value, 3) for value in projections}
    if len(rounded) < 2:
        return "MPR volume processing requires nonzero slice normal and slice span"
    return ""
...
def mpr_geometry_error(slices: list) -> str:
    ...
    stack_error = _volume_stack_geometry_error(slices)
    if stack_error:
        return stack_error
...
def ensure_projection_inputs(datasets: list, source_manifest: dict | None) -> tuple[list, str]:
    ...
    for ds in datasets:
        try:
            ...
            return [], "projection reconstruction requires numeric NumberOfFrames"
        if frame_count > 1:
            return [], "projection reconstruction does not support multi-frame DICOM inputs"
        try:
            shape = getattr(ds.pixel_array, "shape", ())
        except Exception as exc:
            return [], f"projection reconstruction requires decoded pixel data: {exc}"
        if len(shape) != 2 or not all(int(value) > 0 for value in shape):
            return [], "projection reconstruction expects one 2D pixel frame per DICOM"
...
def _registration_stack_for_uid(
    ...
    matches = [(key, group) for key, group in groups.items() if key[1] == series_uid]
    if not matches:
        return [], f"registration {role} series UID was not found in uploaded CT/MR volume stacks"
    if len(matches) > 1:
        return [], f"registration {role} series UID maps to multiple image shapes"
    slices, error = expand_primary_stack(matches[0][1])
    if error:
        return [], f"registration {role} stack: {error}"
    if len(slices) < 2:
        return [], f"registration {role} stack requires at least two slices"
...
def ensure_registration_inputs(datasets: list, source_manifest: dict | None) -> tuple[list, list, str]:
    if not source_manifest:
        return [], [], "registration requires voxellab.source.json source manifest"
    groups = _eligible_dicom_groups(datasets, "auto")
    series_uids = {key[1] for key in groups}
    errors = registration_manifest_errors(source_manifest, series_uids)
    if errors:
        return [], [], "; ".join(errors)
    registration = source_manifest["registration"]
    fixed, fixed_error = _registration_stack_for_uid(groups, str(registration["fixedSeriesUID"]), "fixed")
    if fixed_error:
        return [], [], fixed_error
    moving, moving_error = _registration_stack_for_uid(groups, str(registration["movingSeriesUID"]), "moving")
    if moving_error:
        return [], [], moving_error
    return fixed, moving, ""
