...
def process_series(series: dict) -> None:
    ...
    # Distinguish the crude bounding-box/tissue-class path from the trained DL
    # models (synthseg/totalseg set their own anatomySource) so the UI frames
    # honesty correctly per source. setdefault: never clobber a real model tag.
    series.setdefault("anatomySource", "heuristic")
