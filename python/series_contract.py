...
def validate_volume_geometry(series_path: str, series: dict[str, Any]) -> list[str]:
    ...
    if claims_display_volume(series) and series.get("frameOfReferenceUIDConsistent") is False:
        errors.append(f"{series_path}.frameOfReferenceUIDConsistent: display-volume series must have one consistent FrameOfReferenceUID")
    if claims_display_volume(series) and series.get("slicePositionsDistinct") is False:
        errors.append(f"{series_path}.slicePositionsDistinct: display-volume series must have distinct patient-space slice positions")
...
def merge_manifest_path(
    ...
    *,
    ...
    manifest = load_json(manifest_path) if manifest_path.exists() else {"patient": "anonymous", "studyDate": "", "series": []}
