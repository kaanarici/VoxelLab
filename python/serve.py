...
MAX_CONVERT_BODY_BYTES = 256 * 1024 * 1024  # vendor microscopy uploads
...
def proxy_modal_json(function_name: str, payload: dict, timeout: int = 60) -> tuple[int, dict]:
    ...
    try:
        with urlrequest.urlopen(req, timeout=timeout) as response:
            raw = response.read(MAX_MODAL_JSON_BODY_BYTES + 1)
            if len(raw) > MAX_MODAL_JSON_BODY_BYTES:
        ...
        try:
            raw = exc.read(MAX_MODAL_JSON_BODY_BYTES + 1)
            if len(raw) > MAX_MODAL_JSON_BODY_BYTES:
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def _write_request_body_to_temp(self, ext: str, length: int) -> str:
        ...
        try:
            with tempfile.NamedTemporaryFile(suffix=ext, delete=False) as fin:
                ...
                remaining = length
                while remaining > 0:
                    chunk = self.rfile.read(min(CONVERT_STREAM_CHUNK_BYTES, remaining))
                    if not chunk:
    ...
    def _convert_microscopy_upload(self, ext: str, length: int) -> None:
        ...
        try:
            ...
            in_path = self._write_request_body_to_temp(ext, length)
            out_path = f"{in_path}.ome.tiff"
            _ = convert_to_ome_tiff(in_path, out_path)
            with open(out_path, "rb") as fout:
                self.send_response(200)
                self.send_header("Content-Type", "image/tiff")
                self.send_header("Content-Length", str(os.path.getsize(out_path)))
                self.end_headers()
                while True:
                    chunk = fout.read(CONVERT_STREAM_CHUNK_BYTES)
                    if not chunk:
                        break
                    try:
            ...
            self._json(501, {"error": str(exc) or "microscopy converter unavailable"})
        except ValueError as exc:
            self._json(400, {"error": str(exc)})
        except Exception as exc:
            traceback.print_exc()
            self._json(500, {"error": f"conversion failed: {exc}"})
        finally:
    ...
    def do_POST(self):
        ...
        if parsed.path == "/api/microscopy/convert":
            ...
            if ext not in SUPPORTED_CONVERT_EXTENSIONS:
                self._json(400, {"error": "unsupported microscopy format"})
                return
            try:
                length = int(self.headers.get("Content-Length") or 0)
            except ValueError:
            ...
            if length <= 0:
                ...
                return
            if length > MAX_CONVERT_BODY_BYTES:
                self._json(413, {"error": "file too large"})
                return
            self._convert_microscopy_upload(ext, length)
            return
        ...
        if parsed.path == "/api/proxy-asset":
            ...
            try:
                with urlrequest.urlopen(
                    ...
                    self.end_headers()
                    while True:
                        chunk = response.read(CONVERT_STREAM_CHUNK_BYTES)
                        if not chunk:
                            break
                        try:
                            _ = self.wfile.write(chunk)
