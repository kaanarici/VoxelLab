...
test('registerProjectionSet preserves returned cloud projection metadata', () => {
  ...
  assert.equal(record.projectionKind, 'parallel-beam');
  assert.equal(record.projectionCount, 1);
  assert.equal(record.reconstructionStatus, 'reconstructed');
  assert.equal(manifest.projectionSets[0].projectionKind, 'parallel-beam');
  assert.equal(manifest.projectionSets[0].projectionCount, 1);
});
