...
const {
  configureSlimSAMOrtRuntime,
  slimsamResizeLogits,
} = await import('../js/overlay/slimsam-inference.js');
...
test('configureSlimSAMOrtRuntime keeps inference within the strict worker policy', () => {
  const ort = { env: { wasm: {} } };
  configureSlimSAMOrtRuntime(ort);
  assert.equal(ort.env.wasm.wasmPaths, '/node_modules/onnxruntime-web/dist/');
