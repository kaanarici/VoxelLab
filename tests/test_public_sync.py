        ...
        json.dumps(
            {
                "scripts": {
                    ...
                    "check:py:types": "node scripts/run_python.mjs -m basedpyright",
                    "check:py:types:boundaries": "node scripts/run_python.mjs -m basedpyright -p pyright.boundaries.json",
...
def test_public_export_preserves_desktop_release_proof_files(tmp_path):
    ...
    assert (export_dir / ".github" / "assets" / "voxellab-viewer.jpg").exists()
    assert (export_dir / ".github" / "ISSUE_TEMPLATE" / "bug_report.yml").exists()
    assert (export_dir / ".github" / "ISSUE_TEMPLATE" / "feature_request.yml").exists()
    assert (export_dir / ".github" / "ISSUE_TEMPLATE" / "config.yml").exists()
...
def test_public_export_clean_rejects_patient_data_or_nonanonymous_manifest(tmp_path):
    ...
    (data_dir / "patient-scan.dcm").rename(tmp_path / "patient-scan.discarded")
...
def test_public_export_allows_only_digest_verified_synthetic_dicom_fixtures(tmp_path):
    ...
    for rel in sync_public_repo.SYNTHETIC_IMAGING_FIXTURE_SHA256:
        source = ROOT / rel
        target = tmp_path / rel
    ...
    canonical_rel = next(iter(sync_public_repo.SYNTHETIC_IMAGING_FIXTURE_SHA256))
    canonical = tmp_path / canonical_rel
    original = canonical.read_bytes()
    canonical.write_bytes(b"content no longer matches the synthetic fixture")
    try:
        sync_public_repo.assert_public_export_clean(tmp_path)
    except RuntimeError as exc:
        assert canonical_rel in str(exc)
    else:
        raise AssertionError("expected modified allowlisted DICOM to be rejected")
    canonical.write_bytes(original)

    leaked = tmp_path / "tests" / "real-patient.dcm"
    leaked.write_bytes(b"not an allowlisted synthetic fixture")
    try:
        sync_public_repo.assert_public_export_clean(tmp_path)
    except RuntimeError as exc:
