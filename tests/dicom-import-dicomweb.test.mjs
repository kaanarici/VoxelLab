...
async function freshImportModule() {
  const url = new URL(`../js/dicom/dicomweb-import.js?t=${Date.now()}-${Math.random()}`, import.meta.url);
...
test('importDicomwebSeries cancels its in-flight DICOMweb request before frame decoding', async () => {
  const { importDicomwebSeries } = await freshImportModule();
  const controller = new AbortController();
  let observedSignal = null;
  let markRequestStarted;
  const requestStarted = new Promise(resolve => { markRequestStarted = resolve; });
  const pending = importDicomwebSeries({
    wadoBase: 'https://pacs.example/wado-rs',
    studyUID: '1.2.cancel.study',
    seriesUID: '1.2.cancel.series',
    sessionId: 'import-cancel-session',
