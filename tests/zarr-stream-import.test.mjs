...
function arrayMeta({ width, height, compressor = BLOSC_COMPRESSOR, dtype = '<u2' } = {}) {
  return {
    zarr_format: 2,
...
function rootAttrs(scaleLevel2 = [1, 1, 2, 2]) {
  return {
    ome: {
      version: '0.4',
      multiscales: [{
...
function mockFetch({ scaleLevel2 = [1, 1, 2, 2], coarsestCompressor = BLOSC_COMPRESSOR, calls = [] } = {}) {
  const routes = new Map([
    [`${BASE_URL}/.zattrs`, () => jsonResponse(rootAttrs(scaleLevel2))],
    ...
    [`${BASE_URL}/2/.zarray`, () => jsonResponse(arrayMeta({ width: PLANE_WIDTH, height: PLANE_HEIGHT, compressor: coarsestCompressor }))],
...
test('streams the coarsest fitting level, builds a real plane, and returns a valid series entry', async () => {
  ...
  try {
    ...
    assert.match(provenance, /OME-Zarr streamed · level 3\/3 · ×4 downsample · blosc\(lz4, byte-shuffle\)/);
...
test('fails closed with a named reason when the root multiscales metadata is missing', async () => {
  await assert.rejects(
    streamOmeZarrFromUrl(BASE_URL, {
      fetchImpl: async (url) => (url.endsWith('/.zattrs') ? jsonResponse({ ome: { version: '0.4' } }) : notFound()),
      onProgress: () => {},
    }),
    (error) => {
...
test('streamProvenanceText labels a full-resolution level honestly', () => {
  ...
  assert.match(text, /OME-Zarr streamed · level 1\/1 · full resolution · raw/);
});
