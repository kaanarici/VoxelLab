...
test('accuracy ledger preserves complete artifacts when only one oracle corpus is present', {
  skip: hasDicomFixtures() ? false : 'DICOM accuracy fixtures excluded from this build',
}, () => {
  const partialRoot = mkdtempSync(join(tmpdir(), 'voxellab-partial-accuracy-'));
  const jsonPath = join(ROOT, 'accuracy-ledger.json');
  const markdownPath = join(ROOT, 'ACCURACY_LEDGER.md');
  const beforeJson = readFileSync(jsonPath, 'utf8');
  const beforeMarkdown = readFileSync(markdownPath, 'utf8');
  try {
    cpSync(join(FIXTURE_ROOT, 'dicom'), join(partialRoot, 'dicom'), { recursive: true });
    const output = execFileSync(process.execPath, ['scripts/check_accuracy_ledger.mjs'], {
      cwd: ROOT,
      encoding: 'utf8',
      env: { ...process.env, VOXELLAB_ACCURACY_FIXTURE_ROOT: partialRoot },
    });
    assert.match(output, /fixture corpus does not match the committed ledger/i);
    assert.equal(readFileSync(jsonPath, 'utf8'), beforeJson);
    assert.equal(readFileSync(markdownPath, 'utf8'), beforeMarkdown);
  } finally {
    writeFileSync(jsonPath, beforeJson, 'utf8');
    writeFileSync(markdownPath, beforeMarkdown, 'utf8');
    rmSync(partialRoot, { recursive: true, force: true });
  }
});

test('accuracy ledger preserves complete artifacts when both oracle corpora are incomplete', {
  skip: hasDicomFixtures() && hasNiftiFixtures() ? false : 'full private accuracy fixtures required',
}, () => {
  const partialRoot = mkdtempSync(join(tmpdir(), 'voxellab-incomplete-accuracy-'));
  const jsonPath = join(ROOT, 'accuracy-ledger.json');
  const markdownPath = join(ROOT, 'ACCURACY_LEDGER.md');
  const beforeJson = readFileSync(jsonPath, 'utf8');
  const beforeMarkdown = readFileSync(markdownPath, 'utf8');
  try {
    cpSync(join(FIXTURE_ROOT, 'dicom'), join(partialRoot, 'dicom'), { recursive: true });
    const goldenName = readdirSync(join(FIXTURE_ROOT, 'nifti'))
      .find((name) => name.endsWith('.golden.json'));
    assert.ok(goldenName, 'missing private NIfTI golden fixture');
    const golden = JSON.parse(readFileSync(join(FIXTURE_ROOT, 'nifti', goldenName), 'utf8'));
    const partialNiftiRoot = join(partialRoot, 'nifti');
    mkdirSync(partialNiftiRoot);
    copyFileSync(join(FIXTURE_ROOT, 'nifti', goldenName), join(partialNiftiRoot, goldenName));
    copyFileSync(
      join(FIXTURE_ROOT, 'nifti', golden.fixture),
      join(partialNiftiRoot, golden.fixture),
    );
    const output = execFileSync(process.execPath, ['scripts/check_accuracy_ledger.mjs'], {
      cwd: ROOT,
      encoding: 'utf8',
