...
def test_modal_request_validation_accepts_expected_shapes():
    ...
    assert modal_app.validate_processing_mode("rigid_registration") == "rigid_registration"
    assert modal_app.validate_input_kind("dicom_volume_stack") == "dicom_volume_stack"
    assert modal_app.validate_input_kind("calibrated_projection_set", "projection_set_reconstruction") == "calibrated_projection_set"
    assert modal_app.validate_input_kind("calibrated_ultrasound_source", "ultrasound_scan_conversion") == "calibrated_ultrasound_source"
...
def test_modal_standard_segmentation_attempts_cover_ct_and_mr():
    ...
    assert modal_app.totalseg_attempts_for_modality("CT") == [("total", True, modal_app.CT_ROI_SUBSET)]
    assert modal_app.totalseg_attempts_for_modality("MR") == [
...
def test_modal_request_validation_rejects_r2_key_escape_shapes():
    ...
    assert modal_app.validate_input_kind("dicom_volume_stack", "ultrasound_scan_conversion") == ""
    assert modal_app.validate_input_kind("dicom_volume_stack", "rigid_registration") == ""
    assert modal_app.input_kind_error("dicom_volume_stack", "ultrasound_scan_conversion") == "ultrasound_scan_conversion requires calibrated_ultrasound_source input_kind"
    assert modal_app.input_kind_error("dicom_volume_stack", "rigid_registration") == "rigid_registration requires dicom_registration_pair input_kind"
...
def test_modal_registration_validation_selects_fixed_and_moving_stacks():
    modal_app = import_modal_app()
    datasets = [
        FakeDicom(modality="MR", series_uid="fixed", instance=1, position=[0, 0, 0]),
        FakeDicom(modality="MR", series_uid="fixed", instance=2, position=[0, 0, 1]),
        FakeDicom(modality="MR", series_uid="moving", instance=1, position=[0, 0, 0]),
        FakeDicom(modality="MR", series_uid="moving", instance=2, position=[0, 0, 1]),
    ]
    manifest = {
        "sourceKind": "registration",
        "registration": {
            "fixedSeriesUID": "fixed",
            "movingSeriesUID": "moving",
            "transform": "rigid",
        },
    }

    fixed, moving, error = modal_app.ensure_registration_inputs(datasets, manifest)
...
def test_modal_projection_validation_rejects_multiframe_projection_dicom():
    ...
    selected, error = modal_app.ensure_projection_inputs([ds], manifest)
...
def test_modal_mpr_geometry_validation_rejects_duplicate_slice_positions():
    ...
    slices = [
        FakeDicom(instance=1, position=[0, 0, 0]),
        FakeDicom(instance=2, position=[0, 0, 0]),
        FakeDicom(instance=3, position=[0, 0, 1]),
    ]
...
def test_modal_mpr_geometry_validation_rejects_mixed_frames_of_reference():
    modal_app = import_modal_app()
    slices = [
        FakeDicom(instance=1, position=[0, 0, 0], frame_uid="for-A"),
        FakeDicom(instance=2, position=[0, 0, 1], frame_uid="for-B"),
    ]
...
def test_normalize_upload_items_rejects_oversized_structured_batches():
    ...
    items, error = modal_app.normalize_upload_items({
        "items": [
            {"upload_id": f"f{index:06d}", "filename": f"IM{index:06d}"}
...
def test_normalize_upload_items_rejects_oversized_legacy_batches():
    ...
    items, error = modal_app.normalize_upload_items({
        "filenames": [f"IM{index:06d}" for index in range(MAX_UPLOAD_ITEMS + 1)]
    })
