...
test('ROI delete removes the stored shape by stable id', () => {
  ...
  assert.equal(roiEntriesForSlice(state, state.manifest.series[0], 2).length, 0);
});
...
test('angle delete renders a visible control and removes the group immediately', () => {
  ...
  setAngleEntriesForSlice(state, state.manifest.series[0], 0, [{
    id: 1,
    p1: { x: 10, y: 30 },
    vertex: { x: 30, y: 30 },
    p3: { x: 30, y: 10 },
  ...
  assert.equal(angleEntriesForSlice(state, state.manifest.series[0], 0).length, 0);
