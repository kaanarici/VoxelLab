...
const PUBLIC_SAMPLE_EVIDENCE = Object.freeze([
  ...
  },
  {
    ...
    coverage: 'public OME-NGFF metadata proves axes and units',
    boundary: 'metadata-only public proof',
  },
]);
...
const PUBLIC_RELEASE_PROOF_STEPS = Object.freeze(
  FULL_PROOF_STEPS.filter(step => !PUBLIC_RELEASE_OMITTED_IDS.includes(step.id)),
);
const FULL_RESEARCHER_WORKFLOW = Object.freeze([
  ...
  outcome: `${id} outcome`,
  status: 'passed',
  evidenceLanes: [{ id: 'node-contracts', status: 'passed' }],
})));
const PUBLIC_RELEASE_RESEARCHER_WORKFLOW = Object.freeze([
  {
    id: 'desktop-install-open',
    outcome: 'desktop-install-open outcome',
    status: 'passed',
    evidenceLanes: [
      { id: 'desktop-package-contract', status: 'passed' },
      { id: 'release-download-contract', status: 'passed' },
      { id: 'electron-desktop-intake', status: 'passed' },
    ],
  },
  {
    id: 'mixed-folder-triage',
    outcome: 'mixed-folder-triage outcome',
    status: 'passed',
    evidenceLanes: [
      { id: 'node-contracts', status: 'passed' },
      { id: 'browser-user-flows', status: 'passed' },
      { id: 'electron-desktop-intake', status: 'passed' },
    ],
  },
  {
    id: 'supported-open-calibration-provenance',
    outcome: 'supported-open-calibration-provenance outcome',
    status: 'passed',
    evidenceLanes: [
      { id: 'node-contracts', status: 'passed' },
      { id: 'public-sample-fixtures', status: 'passed' },
      { id: 'browser-user-flows', status: 'passed' },
      { id: 'electron-desktop-intake', status: 'passed' },
    ],
  },
  {
    ...
    outcome: 'microscopy-measure-export-recipe outcome',
    status: 'passed',
    evidenceLanes: [
      { id: 'node-contracts', status: 'passed' },
      { id: 'public-sample-fixtures', status: 'passed' },
  ...
  },
  {
    ...
    outcome: 'honest-failure-boundaries outcome',
    status: 'partial',
    evidenceLanes: [
      { id: 'node-contracts', status: 'passed' },
      { id: 'validation-matrix-contract', status: 'omitted' },
      { id: 'browser-user-flows', status: 'passed' },
      { id: 'electron-desktop-intake', status: 'passed' },
    ],
  },
  {
    ...
    outcome: 'public-release-handoff outcome',
    status: 'partial',
    evidenceLanes: [
      { id: 'public-export-contract', status: 'omitted' },
      { id: 'release-download-contract', status: 'passed' },
    ],
  },
]);
...
function writePublicReleaseReport(root, overrides = {}) {
  writeFileSync(path.join(root, 'voxellab-lab-readiness', 'lab-readiness-report.json'), JSON.stringify({
    gate: 'VoxelLab lab readiness',
    status: 'passed',
    durationMs: PUBLIC_RELEASE_PROOF_STEPS.length,
    repo: CLEAN_REPO,
    proofCoverage: {
      scope: 'partial',
      totalLanes: FULL_PROOF_LANE_COUNT,
      includedLanes: PUBLIC_RELEASE_PROOF_STEPS.length,
      omittedLanes: PUBLIC_RELEASE_OMITTED_IDS.length,
      omittedIds: PUBLIC_RELEASE_OMITTED_IDS,
    },
    researcherWorkflow: PUBLIC_RELEASE_RESEARCHER_WORKFLOW,
    steps: PUBLIC_RELEASE_PROOF_STEPS,
    ...overrides,
  }));
}
...
test('release assets check accepts the sanitized public-release lab evidence shape', async () => {
  const root = await makeReleaseRoot();
  writePublicReleaseReport(root);
  const result = checkReleaseAssets(root);
  assert.equal(result.fileCount, 6);
  assert.ok(result.files.includes('voxellab-lab-readiness/lab-readiness-report.json'));
});
...
test('release assets check rejects partial readiness reports', async () => {
  ...
  assert.throws(() => checkReleaseAssets(root), /full gate or the expected public-release partial gate/);
});

test('release assets check rejects public-release partial reports with missing release proof lanes', async () => {
  const root = await makeReleaseRoot();
  writePublicReleaseReport(root, {
    proofCoverage: {
      scope: 'partial',
      totalLanes: FULL_PROOF_LANE_COUNT,
      includedLanes: PUBLIC_RELEASE_PROOF_STEPS.length - 1,
      omittedLanes: PUBLIC_RELEASE_OMITTED_IDS.length,
      omittedIds: PUBLIC_RELEASE_OMITTED_IDS,
    },
    steps: PUBLIC_RELEASE_PROOF_STEPS.filter(step => step.id !== 'electron-desktop-intake'),
  });
  assert.throws(() => checkReleaseAssets(root), /included lane count must match the release proof gate/);
});

test('release assets check rejects unexpected public-release omissions', async () => {
  const root = await makeReleaseRoot();
  writePublicReleaseReport(root, {
    proofCoverage: {
      scope: 'partial',
      ...
      includedLanes: PUBLIC_RELEASE_PROOF_STEPS.length,
      omittedLanes: PUBLIC_RELEASE_OMITTED_IDS.length,
      omittedIds: ['validation-matrix-contract', 'demo-pack-contract', 'converter-contracts', 'electron-desktop-intake'],
    },
  });
  assert.throws(() => checkReleaseAssets(root), /full gate or the expected public-release partial gate/);
});

test('release assets check rejects public-release workflow evidence with failed lanes', async () => {
  const root = await makeReleaseRoot();
  writePublicReleaseReport(root, {
    researcherWorkflow: PUBLIC_RELEASE_RESEARCHER_WORKFLOW.map(item => (
      item.id === 'mixed-folder-triage'
        ? { ...item, evidenceLanes: item.evidenceLanes.map(lane => lane.id === 'browser-user-flows' ? { ...lane, status: 'failed' } : lane) }
        : item
    )),
  });
  assert.throws(() => checkReleaseAssets(root), /proof lanes must all pass unless they are expected public-release omissions/);
});
