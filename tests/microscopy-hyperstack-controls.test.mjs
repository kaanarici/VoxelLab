...
} = await import('../js/microscopy-hyperstack-controls.js');
...
test('renderMicroscopyHyperstackControls labels unknown spacing as uncalibrated', () => {
  ...
  assert.equal(calibration.title, 'missing_xy_physical_size');
});
...
test('activateMicroscopyStackPosition swaps the active local stack without changing series', () => {
  installDom();
  const host = hostFixture();
