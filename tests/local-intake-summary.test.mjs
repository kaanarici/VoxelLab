...
test('browser local intake keeps VoxelLab source manifests with DICOM selections', async () => {
  const sourceManifest = {
    name: 'voxellab.source.json',
    webkitRelativePath: 'study/voxellab.source.json',
    async text() {
      return JSON.stringify({ sourceKind: 'projection' });
    },
  };
  const intake = await summarizeLocalIntake([
    { name: 'projection-1.dcm', webkitRelativePath: 'study/projection-1.dcm' },
    sourceManifest,
  ]);

  assert.deepEqual(intake.files.map(file => file.name), ['projection-1.dcm', 'voxellab.source.json']);
  assert.deepEqual(intake.counts, { openable: 1, convertible: 0, sidecar: 1 });
  assert.deepEqual(intake.formatItems.sidecar.map(file => file.formatLabel), ['VoxelLab source manifest']);
