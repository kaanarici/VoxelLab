...
test('desktop local API proxy forwards helper requests to localhost', async () => {
  ...
  const response = await handleDesktopLocalApiRequest(
    new Request('voxellab://app/api/ask/stream?x=1', {
      method: 'POST',
      headers: {
    ...
    }),
    {
      fetch: async (url, init) => {
        ...
        seen.body = JSON.parse(Buffer.from(init.body).toString('utf8'));
...
test('desktop converter jobs require configured absolute tools and write provenance', async () => {
  ...
  assert.equal(provenance.outputFiles[0].entryCount, 1);
  assert.deepEqual(provenance.outputFiles[0].entries, ['source.txt']);
