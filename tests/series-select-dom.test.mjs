...
test('RT Dose metadata rows preserve the attachment summary and decoding boundary', () => {
  assert.deepEqual(rtDoseMetadataRows({ slug: 'src' }, [{
    summary: {
    ...
    ['RT Dose attachments', '1'],
    ['RT Dose metadata', 'dimensions 32 × 16 × 4 · scaling 0.001 · units GY · type PHYSICAL · summation PLAN'],
    ['RT Dose boundary', 'metadata only, dose grid is not decoded or rendered'],
  ]);
});
...
test('microscopy metadata panel exposes TIFF sequence ordering provenance', () => {
  ...
  assert.match(selectSeriesDomSource, /microscopyStorageProvenanceText/);
  assert.match(selectSeriesDomSource, /\['Storage', storageProvenanceText\]/);
