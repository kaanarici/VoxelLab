...
function trackedBinaryFileLike(path, bytes, tracker) {
  const buffer = bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength);
  return {
    name: path.split('/').pop(),
    webkitRelativePath: path,
...
async function tinyConsolidatedOmeZarrFiles(options = {}) {
...
function manyChunkOmeZarrFiles({ tracker }) {
  const width = 16;
  const height = 4;
  const files = [
    fileLike('cells.zarr/.zattrs', {
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      files.push(trackedBinaryFileLike(`cells.zarr/0/0.${y}.${x}`, Buffer.from([y * width + x]), tracker));
    }
  }
  return files;
}
...
test('parseOmeZarrFiles bounds concurrent uncompressed zarr v2 chunk reads', async () => {
  const restoreDocument = installCanvasStub();
  ...
  try {
    ...
    assert.ok(tracker.maxActive <= 8, `expected at most 8 concurrent chunk reads, saw ${tracker.maxActive}`);
  } finally {
    restoreDocument();
  }
});
...
test('parseOmeZarrFiles falls back when a required uncompressed zarr v2 chunk is missing', async () => {
  ...
  assert.equal(parsed.status.includes('No image series was added'), true);
