...
const { decodePixelData, transferSyntaxInfo, isDecodable } = await import('../js/dicom/dicom-codecs.js');
...
function assertGeometryRecord(actual, expected) {
  assert.equal(actual.kind, expected.kind);
  assert.deepEqual(actual.dimensions, expected.dimensions);
  approx(actual.spacingMm.row, expected.spacingMm.row);
  approx(actual.spacingMm.col, expected.spacingMm.col);
  approx(actual.spacingMm.slice, expected.spacingMm.slice);
  assert.deepEqual(actual.sliceSpacingStatsMm, expected.sliceSpacingStatsMm);
  assert.equal(actual.slicePositionsDistinct, expected.slicePositionsDistinct);
  assert.deepEqual(actual.orientation, expected.orientation);
  assert.deepEqual(actual.firstIPP, expected.firstIPP);
  assert.deepEqual(actual.lastIPP, expected.lastIPP);
  approxMatrix(actual.affineLps, expected.affineLps);
  assert.equal(actual.frameOfReferenceUID, expected.frameOfReferenceUID);
  assert.equal(actual.frameOfReferenceUIDConsistent, expected.frameOfReferenceUIDConsistent);
...
for (const caseData of FIXTURE.sharedContract.sliceAxisAlignmentFromSeries) {
  test(`geometry contract: sliceAxisAlignmentFromSeries/${caseData.id}`, () => {
    approx(sliceAxisAlignmentFromSeries(caseData.series), caseData.expected);
  });
}
...
for (const caseData of FIXTURE.sharedContract.compareGroup) {
  test(`geometry contract: compareGroup/${caseData.id}`, () => {
    ...
    if (Object.hasOwn(caseData, 'expected')) {
      assert.equal(result, caseData.expected);
    } else {
      assert.match(result, new RegExp(`^${caseData.expectedPrefix}`));
    }
  });
}
...
for (const caseData of FIXTURE.sharedContract.buildGeometryRecord) {
  test(`geometry contract: buildGeometryRecord/${caseData.id}`, () => {
    ...
    assertGeometryRecord(record, caseData.expected);
  });
}
...
test('isOrthonormalImagePlane rejects non-perpendicular row and column axes', () => {
  assert.equal(isOrthonormalImagePlane([1, 0, 0, 0, 1, 0]), true);
  assert.equal(isOrthonormalImagePlane([2, 0, 0, 0, 1, 0]), false);
  assert.equal(isOrthonormalImagePlane([1, 0, 0, 0.2, 1, 0]), false);
});

test('geometryFromDicomMetas uses SpacingBetweenSlices when IPP spacing is unavailable', () => {
  const geometry = geometryFromDicomMetas([{
  ...
  assert.equal(geometry.sliceSpacing, 2.4);
  assert.equal(geometry.sliceThickness, 1.2);
});
...
test('transferSyntaxInfo classifies known transfer syntaxes correctly', () => {
  ...
  assert.equal(transferSyntaxInfo('1.2.840.10008.1.2.1.99').category, 'uncompressed');
  ...
  assert.equal(transferSyntaxInfo('1.2.840.10008.1.2.4.80').category, 'lossless');
  assert.equal(transferSyntaxInfo('1.2.840.10008.1.2.4.81').category, 'lossy-quantitative');
...
test('isDecodable returns true for lossless and uncompressed, false for lossy-display', () => {
  ...
  assert.equal(isDecodable('1.2.840.10008.1.2.4.80'), true);
