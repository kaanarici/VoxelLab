...
test('cloud upload fails immediately when Modal dispatch outcome is unknown', async (t) => {
  let statusCalls = 0;
  installFastTimers(t);
  installCloudFetch(t, () => {
    statusCalls += 1;
    return { status: 'dispatch_unknown', detail: 'Modal dispatch outcome is unknown' };
  });
  ...
  await assert.rejects(
    () => cloud.uploadAndProcess([{ name: 'slice-1.dcm' }]),
    /Modal dispatch outcome is unknown/,
  );
  assert.equal(statusCalls, 1);
});
