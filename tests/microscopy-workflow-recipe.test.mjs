...
test('applyMicroscopyWorkflowRecipe restores embedded angle measurements without duplicating replay', () => {
  ...
  const reloadedHost = fixture();
  const reloadedSeries = reloadedHost.manifest.series[0];
  reloadedSeries.slug = 'cells_recipe_reloaded';
  reloadedHost._localStacks.cells_recipe_reloaded = reloadedHost._localStacks.cells_recipe;
  reloadedHost._localMicroscopyStacks.cells_recipe_reloaded = reloadedHost._localMicroscopyStacks.cells_recipe;
  resetHost(reloadedHost);
  setAngleEntriesForSlice(state, reloadedSeries, 1, [{
    id: 11,
    label: 'branch-angle',
    p1: { x: 6, y: 4 },
    vertex: { x: 2, y: 4 },
  ...
  assert.deepEqual(applyMicroscopyWorkflowRecipe(recipe, state), { ok: true, code: '', message: '' });
  assert.equal(angleEntriesForSlice(state, reloadedSeries, 1).length, 1);

  const conflictingHost = fixture();
  const conflictingSeries = conflictingHost.manifest.series[0];
  conflictingSeries.slug = 'cells_recipe_conflict';
  conflictingHost._localStacks.cells_recipe_conflict = conflictingHost._localStacks.cells_recipe;
  conflictingHost._localMicroscopyStacks.cells_recipe_conflict = conflictingHost._localMicroscopyStacks.cells_recipe;
  resetHost(conflictingHost);
  setAngleEntriesForSlice(state, conflictingSeries, 1, [{
    id: 11,
  ...
  assert.deepEqual(applyMicroscopyWorkflowRecipe(recipe, state), { ok: true, code: '', message: '' });
  assert.equal(angleEntriesForSlice(state, conflictingSeries, 1).length, 2);
});
