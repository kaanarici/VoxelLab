...
test('uncalibrated medical stacks name missing spacing before generic geometry', () => {
  assert.match(
    capabilityBlockReason({
      geometryKind: 'imageStack',
      reconstructionCapability: '2d-only',
      ...
      pixelSpacing: [0, 0],
      _spacingKnown: false,
      dicomImportKind: 'image-stack',
    }),
    /DICOM series is missing trusted pixel spacing/,
  );
  assert.match(
    capabilityBlockReason({
      width: 8,
      ...
      pixelSpacing: [0, 0],
      _spacingKnown: false,
      _niftiSpatialUnit: 'unknown',
    }),
    /NIfTI series is uncalibrated because its spatial unit is unknown/,
  );
  assert.match(
    capabilityBlockReason({
      geometryKind: 'imageStack',
...
test('mixed DICOM frames of reference block explicit volume claims', () => {
  const series = {
    geometryKind: 'volumeStack',
    reconstructionCapability: 'display-volume',
  ...
  assert.equal(reconstructionCapabilityForSeries(series), '2d-only');
  assert.equal(canUseMpr3D(series), false);
  assert.match(capabilityBlockReason(series), /frames of reference/i);
});

test('sheared slice axes block MPR/3D instead of rendering an orthogonal cube', () => {
  const series = {
    geometryKind: 'volumeStack',
    reconstructionCapability: 'display-volume',
    width: 512,
...
test('non-orthogonal image-plane axes block MPR/3D', () => {
  const series = {
    geometryKind: 'volumeStack',
    reconstructionCapability: 'display-volume',
    width: 512,
...
test('only resource-safe calibrated microscopy stacks gain the shared MPR/3D path', () => {
  const calibrated = {
    ...
    height: 1024,
    slices: 12,
    pixelSpacing: [0.00025, 0.00025],
    sliceThickness: 0.001,
    sliceSpacing: 0.001,
    firstIPP: [0, 0, 0],
    lastIPP: [0, 0, 0.011],
    orientation: [1, 0, 0, 0, 1, 0],
    microscopy: { sizeZ: 12, sizeC: 2, sizeT: 1, physicalUnit: 'µm', volumeEligible: true },
  };
  assert.equal(canUseMpr3D(calibrated), true);
  ...
  const series = {
    ...
    pixelSpacing: [0.00025, 0.00025],
    sliceThickness: 0.001,
    microscopy: { sizeZ: 12, sizeC: 2, sizeT: 1, physicalUnit: 'µm', volumeEligible: false },
  };

  assert.equal(geometryKindForSeries(series), 'microscopyStack');
  assert.equal(reconstructionCapabilityForSeries(series), '2d-only');
  assert.equal(capabilityLabel(series), 'Microscopy stack · 2D only');
  assert.equal(canUseMpr3D(series), false);
  assert.match(capabilityBlockReason(series), /local volume safety budget/i);
});
