...
test('microscopy stacks stay in the 2D hyperstack path', () => {
  const series = {
    ...
    microscopy: { sizeZ: 12, sizeC: 2, sizeT: 1, physicalUnit: 'µm' },
  };
  ...
  assert.equal(canUseMpr3D(series), false);
  assert.match(capabilityBlockReason(series), /Z\/channel\/time/i);
});
