...
test('sampleObliqueCompositeSlice aligns discrete labels with the MIP slab winner', () => {
  const projection = createMprProjection(
    { mode: 'mip', slabThicknessMm: 2 },
    { row: 1, col: 1, slice: 1 },
    { axisU: [1, 0, 0], axisV: [0, 1, 0] },
  );
  const sampled = sampleObliqueCompositeSlice(
    ...
    1,
    {
      segVoxels: Uint8Array.from([0, 1, 3]),
      regionVoxels: Uint8Array.from([0, 9, 42]),
    },
    null,
    undefined,
    projection,
  ...
  assert.equal(sampled.baseBytes[0], 200);
  assert.equal(sampled.segBytes[0], 3);
  assert.equal(sampled.regionBytes[0], 42);
});
...
test('obliqueRasterSize caps large panes without changing physical aspect', () => {
  const raster = obliqueRasterSize({ width: 1800, height: 450 }, 1024);
  assert.deepEqual(raster, { width: 1024, height: 256 });
  assert.equal(raster.width / raster.height, 4);
});
