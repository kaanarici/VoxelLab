...
const {
  ...
  roiResultsCsv,
  setRoiResultLabel,
} = await import('../js/roi-results.js');
...
test('roiResultsCsv includes Fiji-style result columns and escapes values', () => {
  ...
  assert.match(csv, /^roi,roi_object_id,series,series_slug,slice,z_index0,kind,label,x_px,y_px,count,length_um,length_mm,length_px,channel,channel_index0,time,time_index0,area_um2,area_mm2,pixels,mean,std,min,max,value_unit,value_source,created_at\n/);
  assert.match(csv, /9,roi:quoted_series\|3:9,"'\t=cells, GFP",quoted_series,4,3,polygon,"'=nucleus, GFP",,,,,,,"DAPI ""blue""",0,2,1,12.5,0.0000125,50,20,2,1,40,8-bit,display_8bit,2023-11-14T22:13:20.000Z\n$/);
});
...
test('roiResultsCsv keeps non-microscopy area headers unique', () => {
  ...
  assert.match(csv, /^roi,roi_object_id,series,series_slug,slice,z_index0,kind,label,x_px,y_px,count,length_display_mm,length_mm,length_px,channel,channel_index0,time,time_index0,area_display_mm2,area_mm2,pixels,mean,std,min,max,value_unit,value_source,created_at\n/);
...
test('roiResultRows exports calibrated line measurements with C/T provenance and length fields', () => {
  ...
  assert.match(csv, /6,measure:cell_lengths\|0:6,cell lengths,cell_lengths,1,0,line,Line 6,6,4,,4,0.004,,GFP,1,3,2,,,,,,,,,linear_measurement,2023-11-14T22:13:20.000Z\n$/);
...
test('roiResultRows exports C/T-scoped point count ROIs with pixel coordinates', () => {
  ...
  const csv = roiResultsCsv([row], state.manifest.series[0]);
  assert.match(csv, /4,roi:cell_counts\|0:4,cell counts,cell_counts,1,0,point,ROI 4,6,9,1,,,,GFP,1,4,3,,,1,77,0,77,77,8-bit,display_8bit,2023-11-14T22:13:20.000Z\n$/);
