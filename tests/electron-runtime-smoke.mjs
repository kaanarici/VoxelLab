...
async function calibratedOmePersistenceKey(name) {
  const restoreCanvas = installMicroscopyCanvasStub();
  try {
    const [result] = await parseMicroscopyFiles([calibratedOmeFileLike(name)]);
    const manifest = JSON.parse(await fs.readFile(new URL('../data/manifest.json', import.meta.url), 'utf8'));
    return seriesPersistenceKey(normalizeSeriesEntryForManifest(manifest, result.entry), manifest);
  } finally {
    restoreCanvas();
  }
}
...
test('Electron runtime delivers queued launch paths after renderer bridge readiness', async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('#upload-modal.visible').count(), 0);
    ...
    assert.match(dialog.body || '', /Choose other files/);
    await assertShellFits(page);
    assert.deepEqual(pageErrors, []);
  } finally {
    await closeApp(app);
...
test('Electron runtime bounds direct unsupported launch file samples', async () => {
  ...
  try {
    ...
    assert.equal(dialog.title, 'Unsupported desktop selection');
    assert.match(dialog.body || '', /Skipped \(unsupported\)/);
    assert.match(dialog.body || '', /notes-0\.md/);
    assert.match(dialog.body || '', /notes-4\.md/);
    assert.match(dialog.body || '', /\+2 more files/);
    assert.match(dialog.body || '', /Try DICOM, NIfTI/);
...
test('Electron runtime reports skipped files for unsupported folder launches', async () => {
  ...
  try {
    ...
    assert.match(dialog.body || '', /notes\.md/);
    assert.match(dialog.body || '', /results\.csv/);
    assert.match(dialog.body || '', /Folder warnings/);
    assert.match(dialog.body || '', /No supported files found/);
    await assertShellFits(page);
    assert.deepEqual(pageErrors, []);
  } finally {
    await closeApp(app);
...
test('Electron runtime holds a supported DICOM SR launch until its source series opens', async () => {
  ...
  await writeTinySr(srPath, {
    sourceSeriesUID: '1.2.826.0.1.3680043.10.543.999',
    sourceSlug: 'pending-source',
  });
  ...
  try {
    await page.waitForFunction(() => (
      [...document.querySelectorAll('#notify-container .notify-text')]
        .some(item => /Holding 1 derived object in this session/.test(item.textContent || '')
          && /attach automatically when the matching source series is loaded/.test(item.textContent || ''))
    ), null, { timeout: 15_000 });
    assert.equal(await page.locator('#upload-modal.visible').count(), 0);
    assert.equal(await page.locator('#confirm-modal.visible').count(), 0);
    await page.waitForFunction(() => (
      [...document.querySelectorAll('#notify-container .notify-text')]
        .some(item => /Desktop intake:/.test(item.textContent || '')
...
test('Electron runtime imports ROI sidecars with local OME-TIFF launches', async () => {
  ...
  await writeCalibratedOmeTiff(omeTiffPath);
  const persistenceKey = await calibratedOmePersistenceKey(path.basename(omeTiffPath));
  await fs.writeFile(roiPath, encodeImageJRoi({
    kind: 'ellipse',
    label: 'cell-body',
    slice: 1,
...
test('Electron runtime starts configured desktop conversion and reopens the OME-TIFF output', async () => {
  ...
  const convertedFixture = path.join(tempDir, 'converted-fixture.ome.tiff');
  await fs.writeFile(convertible, 'fake czi');
  await writeCalibratedOmeTiff(convertedFixture);
  await fs.writeFile(converter, [
    '#!/usr/bin/env node',
    "import fs from 'node:fs/promises';",
    'const [, , input, output] = process.argv;',
    `await fs.copyFile(${JSON.stringify(convertedFixture)}, output);`,
