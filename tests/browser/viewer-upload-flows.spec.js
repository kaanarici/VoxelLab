...
async function writeTinySr(path, { sourceSeriesUID = '', sourceSlug = 'pending-source' } = {}) {
  const { DicomDict, DicomMetaDictionary } = dcmjs.data;
  const dataset = {
    _meta: {
      FileMetaInformationVersion: new Uint8Array([0, 1]).buffer,
  ...
  if (sourceSeriesUID) {
    dataset.ReferencedSeriesSequence = [{ SeriesInstanceUID: sourceSeriesUID }];
...
function segMetadataInstance({
  ...
  return {
    ...
    '00620001': { vr: 'CS', Value: [segmentationType] },
    '00280008': { vr: 'IS', Value: [2] },
    '00280010': { vr: 'US', Value: [2] },
    '00280011': { vr: 'US', Value: [2] },
...
test('upload modal lists imported cloud results and can reopen one', async ({ page }) => {
  ...
  await expect(page.locator('#upload-modal')).toBeVisible();
  await expect(page.locator('#upload-status')).toContainText('Cloud job is running');
  await expect(page.locator('#series-name')).toHaveText('Local CT');
  await page.locator('#btn-upload').evaluate((button) => button.click());
  await expect(page.locator('#upload-modal')).toBeVisible();
  await expect(page.locator('#upload-cloud-history')).toContainText('Cloud Segmentation Result');
  await expect(page.locator('#series-name')).toHaveText('Local CT');
  await page.locator('#upload-modal').evaluate((modal) => { delete modal.dataset.closeBlocked; });
...
async function routeDerivedDcmjsStub(page) {
  await page.route('**/node_modules/dcmjs/build/dcmjs.es.js', async (route) => {
    await route.fulfill({
      status: 200,
...
test('upload modal keeps the latest dropped selection when an earlier source manifest resolves late', async ({ page }) => {
  ...
  await page.locator('#upload-zone').evaluate((target) => {
    let resolveManifest;
    const manifestText = new Promise(resolve => { resolveManifest = resolve; });
    window.__resolveUploadRaceManifest = () => resolveManifest(JSON.stringify({ sourceKind: 'projection' }));
    window.__uploadRaceManifestStarted = false;
    const sourceManifest = new File(['{}'], 'voxellab.source.json', { type: 'application/json' });
    Object.defineProperty(sourceManifest, 'text', {
      value() {
        window.__uploadRaceManifestStarted = true;
        return manifestText;
      },
    });
    const transfer = new DataTransfer();
    transfer.items.add(new File([new Uint8Array([1, 2, 3, 4])], 'first.dcm', { type: 'application/dicom' }));
    transfer.items.add(sourceManifest);
    const event = new Event('drop', { bubbles: true, cancelable: true });
    Object.defineProperty(event, 'dataTransfer', { value: transfer });
    target.dispatchEvent(event);
  });
  await expect.poll(async () => page.evaluate(() => window.__uploadRaceManifestStarted)).toBe(true);

  await page.locator('#upload-zone').evaluate((target) => {
    const transfer = new DataTransfer();
    transfer.items.add(new File([new Uint8Array(360)], 'second.nii', { type: 'application/octet-stream' }));
    const event = new Event('drop', { bubbles: true, cancelable: true });
    Object.defineProperty(event, 'dataTransfer', { value: transfer });
    target.dispatchEvent(event);
  });
  ...
  const status = page.locator('#upload-status');
  await expect(status).toContainText('Opened (NIfTI)');
  await page.evaluate(() => window.__resolveUploadRaceManifest());
  await page.waitForTimeout(250);
  await expect.poll(async () => status.innerText()).toContain('Opened (NIfTI)');
  await expect(status).not.toContainText('DICOM');
  await expect(status).not.toContainText('VoxelLab source manifest');
});
...
test('upload modal rejects a malformed standalone DICOM SR without implying it was queued', async ({ page }, testInfo) => {
  ...
  const status = page.locator('#upload-status');
  await expect(status).toHaveClass(/is-error/);
  await expect(status).toContainText('Could not parse selected files.');
  await expect(status).toContainText('Failed 1 attempted file: measurements.sr.');
  await expect(status).toContainText('checked 1 file');
  await expect(status).toContainText('selected 1 sidecar (DICOM SR)');
  await expect(status).toContainText('Check format or open the full source folder.');
  await expect(page.locator('#upload-modal')).toBeVisible();
});
...
test('upload modal shows honest format support boundaries without layout overflow', async ({ page }) => {
  ...
  await expect(matrix.locator('.format-capability-row', { hasText: 'OME-TIFF / ImageJ TIFF' }))
    .toContainText('MPR/3D');
  await expect(matrix.locator('.format-capability-row', { hasText: 'ImageJ ROI .roi' }))
    .toContainText('open PolyLine');
  await expect(matrix.locator('.format-capability-row', { hasText: 'SEG / RTSTRUCT / SR / RT Dose' }))
    .toContainText('session queue');
  await expect(page.locator('.upload-copy', {
    hasText: 'generic TIFF sequences stay uncalibrated',
  })).toBeVisible();
  await expect(matrix.locator('.format-capability-row', { hasText: 'OME-Zarr (limited)' }))
    .toContainText('Zarr v3');
  await expect(matrix.locator('.format-capability-row', { hasText: 'TIFF JPEG / BigTIFF / tiled pyramids' }))
    .toContainText('fails closed');
  await expect(matrix.locator('.format-capability-row', { hasText: 'CZI / ND2 / LIF / OIB / OIF / LSM bridge' }))
    .toContainText('scenes, ND2 positions, and LIF images/positions');
...
test('supported local SR waits for and then attaches to its matching source series', async ({ page }, testInfo) => {
  const sourceSeriesUID = '1.2.826.0.1.3680043.10.543.777';
  const srPath = testInfo.outputPath('pending-local-sr/measurements.sr');
  const dicomPath = testInfo.outputPath('pending-local-sr/source.dcm');
  await writeTinySr(srPath, { sourceSeriesUID, sourceSlug: 'source-before-reimport' });
  await writeTinyDicom(dicomPath, { seriesUID: sourceSeriesUID });
  ...
  await expect(page.locator('#notify-container .notify-text', { hasText: 'Holding 1 derived object in this session' }))
    .toContainText('attach automatically when the matching source series is loaded');
  await expect(page.locator('#upload-modal')).toBeHidden();
  ...
  await expect.poll(() => page.evaluate(async () => {
    ...
    const { noteEntriesForSlice } = await import('/js/overlay/annotation-graph.js');
    const series = state.manifest.series[state.seriesIdx];
    return {
      pending: state._pendingDerivedObjects?.length || 0,
      notes: noteEntriesForSlice(state, series, 0).filter(note => note.sourceObjectUID).length,
      derived: Object.keys(state._localDerivedObjects?.[series.slug] || {}).length,
    };
  })).toEqual({ pending: 0, notes: 1, derived: 1 });
  await waitForCanvasPaint(page, '#view');
});
...
test('upload modal can bind a DICOMweb RT Dose series onto an already loaded source study', async ({ page }) => {
  ...
  await expect(page.locator('#meta .meta-row').filter({ hasText: 'RT Dose attachments' })).toContainText('1');
  await expect(page.locator('#meta .meta-row').filter({ hasText: 'RT Dose metadata' }))
    .toContainText('dimensions 32 × 16 × 4 · scaling 0.001 · units GY · type PHYSICAL · summation PLAN');
  await expect(page.locator('#meta .meta-row').filter({ hasText: 'RT Dose boundary' }))
    .toContainText('metadata only, dose grid is not decoded or rendered');
...
test('upload modal can bind DICOMweb SEG, RTSTRUCT, and SR objects onto an already loaded source study', async ({ page }) => {
  ...
  await page.route('https://pacs.example/**', async (route) => {
    ...
    if (requestUrl.pathname === '/studies/1.2.study/series/1.2.series.comparison/metadata') {
      return route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify([enhancedMetadataInstance({
          seriesUID: '1.2.series.comparison',
          sopInstanceUID: '1.2.sop.comparison',
          description: 'DICOMweb comparison',
        })]),
      });
    }
    ...
    if (requestUrl.pathname === '/studies/1.2.study/series/1.2.seg.fractional/metadata') {
      return route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify([segMetadataInstance({
          seriesUID: '1.2.seg.fractional',
          sopInstanceUID: '1.2.seg.fractional.object',
    ...
    if (requestUrl.pathname.includes('/series/1.2.seg.fractional/instances/1.2.seg.fractional.object/frames/')) {
      return route.fulfill({
        status: 200,
        contentType: 'application/octet-stream',
        body: Buffer.from(new Uint8Array([0])),
      });
    }
    if (requestUrl.pathname.endsWith('/frames/1') || requestUrl.pathname.endsWith('/frames/2')) {
      const frame = requestUrl.pathname.endsWith('/frames/1') ? new Uint16Array([1, 2, 3, 4]) : new Uint16Array([5, 6, 7, 8]);
  ...
  await page.locator('#dicomweb-base').fill('https://pacs.example');
  await page.locator('#dicomweb-study').fill('1.2.study');
  await page.locator('#dicomweb-series').fill('1.2.series.comparison');
  await page.locator('#upload-dicomweb-btn').click();

  await expect(page.locator('#series-name')).toHaveText('DICOMweb comparison');
  await expect(page.locator('#series-list li')).toHaveCount(initialCount + 2);
  const beforeRejectedSeg = await page.evaluate(async (loadedSourceSlug) => {
    const { state } = await import('/js/core/state.js');
    const registry = JSON.parse(localStorage.getItem('mri-viewer/derived-objects/v1') || '{"entries":{}}');
    return {
      activeSlug: state.manifest.series[state.seriesIdx]?.slug || '',
      derivedEntries: Object.keys(registry.entries || {}).length,
      seriesCount: state.manifest.series.length,
      sourceHasRegions: Boolean(state.manifest.series.find((series) => series.slug === loadedSourceSlug)?.hasRegions),
    };
  }, sourceSlug);
  expect(beforeRejectedSeg.activeSlug).not.toBe(sourceSlug);
  ...
  await page.locator('#dicomweb-base').fill('https://pacs.example');
  await page.locator('#dicomweb-study').fill('1.2.study');
  await page.locator('#dicomweb-series').fill('1.2.seg.fractional');
  await page.locator('#upload-dicomweb-btn').click();

  await expect(page.locator('#upload-modal')).toBeVisible();
  await expect(page.locator('#upload-status')).toContainText('segmentation_type_unsupported');
  await expect.poll(async () => page.evaluate(async (loadedSourceSlug) => {
    const { state } = await import('/js/core/state.js');
    const registry = JSON.parse(localStorage.getItem('mri-viewer/derived-objects/v1') || '{"entries":{}}');
    return {
      activeSlug: state.manifest.series[state.seriesIdx]?.slug || '',
  ...
  }, sourceSlug)).toEqual(beforeRejectedSeg);
  ...
  await expect.poll(async () => page.evaluate(async (slug) => {
    const { state } = await import('/js/core/state.js');
    const { roiEntriesForSlice } = await import('/js/overlay/annotation-graph.js');
  ...
  await expect.poll(async () => page.evaluate(async (slug) => {
    ...
    const { noteEntriesForSlice } = await import('/js/overlay/annotation-graph.js');
    const series = state.manifest.series.find((entry) => entry.slug === slug);
    return noteEntriesForSlice(state, series, 1)[0]?.text || '';
