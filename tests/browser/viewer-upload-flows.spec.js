...
test('upload modal shows honest format support boundaries without layout overflow', async ({ page }) => {
  ...
  await expect(matrix.locator('.format-capability-row')).toHaveCount(9);
  ...
  await expect(matrix).toContainText('CZI / ND2 / LIF bridge');
  await expect(matrix).toContainText('Optional local converter/plugin path only');
  await expect(matrix).toContainText('no first-party browser Bio-Formats parity claim');
