...
async function writeTinyDicom(path, { instanceNumber = 1, z = 0 } = {}) {
...
test('upload modal advertises supported local image formats in the picker', async ({ page }) => {
  ...
  await expect(page.locator('.upload-zone-title')).toHaveText('Drop images, sidecars, or converter-backed files here');
});
...
test('upload modal summarizes mixed folder triage before import action', async ({ page }, testInfo) => {
  ...
  await expect(status).toContainText('1 openable file (NIfTI), 1 converter-backed file (CZI) and 1 sidecar (Workflow recipe) selected');
  await expect(status).toContainText('after checking 6 files');
  await expect(status).toContainText('skipped 3 unsupported files');
  ...
  await expect(status).toContainText('converter-backed files need configured local readers or an OME-TIFF converter and should be opened separately: mixed-folder/cells.czi');
...
test('upload modal separates browser drag folder read failures from unsupported skips', async ({ page }) => {
  ...
  const status = page.locator('#upload-status');
  await expect(status).toHaveClass(/is-active/);
  await expect(status).toContainText('1 openable file (DICOM) selected after checking 4 files');
  await expect(status).toContainText('skipped 1 unsupported file (study/notes.md)');
  await expect(status).toContainText('1 file read failed (study/missing.ome.tif (not found or unreadable))');
  await expect(status).toContainText('1 folder read failed (Could not read folder: private)');
...
test('upload modal names unsupported folder samples when nothing can open', async ({ page }, testInfo) => {
  ...
  await expect(status).toContainText('No supported image, sidecar, or converter-backed files selected');
  await expect(status).toContainText('after checking 7 files');
  await expect(status).toContainText('skipped 7 unsupported files');
