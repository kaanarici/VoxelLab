...
import { openUploadModal, routeConfig, waitForCanvasPaint } from './microscopy-upload-helpers.mjs';
...
function arrayMeta({ width, height, compressor = BLOSC }) {
  return {
    zarr_format: 2,
...
function zarrRouteBody(relPath, { coarsestCompressor = BLOSC, fixtureBytes }) {
  ...
  if (relPath === '2/.zarray') return { json: arrayMeta({ width: PLANE_WIDTH, height: PLANE_HEIGHT, compressor: coarsestCompressor }) };
  if (relPath === '2/0/0/0/0') return { bytes: fixtureBytes };
  return null;
}

async function mockZarrProxy(page, { coarsestCompressor = BLOSC } = {}) {
  ...
  await page.route(`${ZARR_HOST}/**`, async (route) => {
    ...
    const body = zarrRouteBody(relPath, { coarsestCompressor, fixtureBytes });
    if (!body) {
      await route.fulfill({ status: 404, body: '' });
      return;
    }
...
test('streams a downsampled OME-Zarr pyramid level by URL with honest provenance', async ({ page }, testInfo) => {
  ...
  await expect(streamingRow).toContainText('OME-Zarr streamed · level 3/3 · ×4 downsample');
