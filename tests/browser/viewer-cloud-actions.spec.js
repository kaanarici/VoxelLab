...
import { openUploadModal, routeConfig } from './microscopy-upload-helpers.mjs';
...
test('upload modal can start rigid registration as a cloud action', async ({ page }, testInfo) => {
  ...
  await openUploadModal(page);
  const initialCount = await page.locator('#series-list li').count();
  await page.locator('#upload-file-input').setInputFiles([fixedOne, fixedTwo, movingOne, movingTwo, sourcePath]);
