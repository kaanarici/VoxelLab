...
async function obliqueSnapshot(page) {
  return page.locator('#mpr-ob').evaluate(async (canvas) => {
    ...
    const { obliquePlaneExtentMm } = await import('/js/mpr_oblique.js');
    const ctx = canvas.getContext('2d', { willReadFrequently: true });
    const rect = canvas.getBoundingClientRect();
...
test('MPR axis interaction stays off the slice scrub path and defers oblique paint', async ({ page }, testInfo) => {
  ...
  expect(obliqueDuringZScrub, JSON.stringify({ obliqueBeforeZScrub, obliqueDuringZScrub }))
    .toEqual(obliqueBeforeZScrub);
  const settleEvent = await waitForPerfEvent(page, 'mpr-quality-settle', 1_000);
