...
test('shortcut customizer edits, clears, resets, and blocks duplicate bindings', async ({ page }) => {
  ...
  const response = await page.goto('/', { waitUntil: 'domcontentloaded' });
  expect(response?.ok()).toBe(true);
  await page.waitForFunction(() => Boolean(document.getElementById('btn-help')?.onclick));
  ...
  const askRow = page.locator('.shortcut-row', { hasText: 'Ask AI' });
  await askRow.hover();
  await page.getByLabel('Edit shortcut for Ask AI').click();
  await page.keyboard.press('X');
  await expect(askRow.locator('.shortcut-conflict')).toContainText('Already assigned to Screenshot');
