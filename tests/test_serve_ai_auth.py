...
def test_convert_microscopy_upload_streams_body_to_temp(monkeypatch) -> None:
    ...
    def fake_convert_to_ome_tiff(input_path: str, output_path: str) -> str:
    ...
    monkeypatch.setitem(
        sys.modules,
        "microscopy_convert",
        types.SimpleNamespace(convert_to_ome_tiff=fake_convert_to_ome_tiff),
    )
    captured["headers"] = []
...
def test_convert_microscopy_upload_rejects_short_body(monkeypatch) -> None:
    ...
    monkeypatch.setitem(
        sys.modules,
        "microscopy_convert",
        types.SimpleNamespace(convert_to_ome_tiff=lambda _input_path, _output_path: _output_path),
    )
...
def test_analyze_status_persists_terminal_error_state() -> None:
    ...
    serve.RUNNING.clear()
    serve.RUNNING["scan"] = serve._analysis_status_entry(
        proc=FakeProc(),
        status="running",
        last="starting...",
    )

    serve._stream_tail(FakeProc(), "scan")

    payload = serve.status_payload()["scan"]
...
def test_do_post_ask_forwards_region_payload(monkeypatch) -> None:
    ...
    class FakeAsk:
        @staticmethod
        def ask(slug, slice_idx, question, region=None, provider=None, model=None):
            seen["slug"] = slug
