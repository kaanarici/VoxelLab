...
test('trimCacheEntries enforces the hard entry limit while preserving the active key', async () => {
  const entries = Array.from({ length: 5 }, (_, index) => new Request(`https://assets.example/slice-${index}.png`));
  const cache = {
  ...
  assert.deepEqual(entries.map(entry => entry.url), [
    'https://assets.example/slice-0.png',
    'https://assets.example/slice-3.png',
    'https://assets.example/slice-4.png',
  ]);
});
