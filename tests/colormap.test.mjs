...
const { MR_PRESETS } = await import('../js/core/constants.js');
...
test('bright MR preset raises midtones instead of darkening them', () => {
  const midtone = 128;
  const full = mapWindowLevelByte(midtone, MR_PRESETS.full.window, MR_PRESETS.full.level);
  const bright = mapWindowLevelByte(midtone, MR_PRESETS.bright.window, MR_PRESETS.bright.level);
  const contrast = mapWindowLevelByte(midtone, MR_PRESETS.contrast.window, MR_PRESETS.contrast.level);

  assert.ok(bright > full, `bright=${bright} should be above full=${full}`);
  assert.ok(bright < contrast, `bright=${bright} should stay less aggressive than contrast=${contrast}`);
});
...
test('display invert is applied inside the shared render LUT', () => {
  state.window = 100;
  state.level = 100;
  state.colormap = 'grayscale';
