...
const {
  ...
  channelMetadata,
  datasetSpacingMm,
} = await import('../js/microscopy/microscopy-dataset-model.js');
...
test('buildMicroscopyDataset keeps missing xy calibration explicit without inventing z warnings for single planes', () => {
  const dataset = buildMicroscopyDataset({
  ...
  assert.deepEqual(dataset.source.warnings, ['missing_xy_physical_size']);
  assert.deepEqual(datasetSpacingMm(dataset), {
    rowMm: 0,
    colMm: 0,
    zMm: 0,
...
test('channelByIndex resolves numeric channel indexes with a non-serialized cache', () => {
  const firstGfp = { index: '1', name: 'GFP original' };
  const secondGfp = { index: 1, name: 'GFP duplicate' };
  const dataset = { channels: [{ index: 0, name: 'DAPI' }, firstGfp, secondGfp] };
  const keysBefore = Object.keys(dataset);

  assert.equal(channelByIndex(dataset, 1), firstGfp);
  assert.equal(channelByIndex(dataset, '1'), firstGfp);
  assert.equal(channelByIndex(dataset, 2), null);
  assert.deepEqual(Object.keys(dataset), keysBefore);

  const replacement = { index: 1, name: 'GFP replacement' };
  dataset.channels = [replacement];

  assert.equal(channelByIndex(dataset, 1), replacement);
});

test('buildMicroscopyDataset warns when z-stack spacing is missing', () => {
  const dataset = buildMicroscopyDataset({
    fileName: 'uncalibrated-stack.tif',
    metadata: { source: 'TIFF sequence', sizeZ: 2, sizeC: 1, sizeT: 1 },
    pages: [
      { width: 1, height: 1, bitsPerSample: 8, samplesPerPixel: 1, pixels: new Float32Array([7]) },
    ],
  });
