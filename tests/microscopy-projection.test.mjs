...
test('projectStack preserves uint32 values beyond Float32 precision in max and arithmetic modes', () => {
  const uint32Planes = [
    { width: 2, height: 1, pixels: new Uint32Array([16_777_216, 4_000_000_000]) },
    { width: 2, height: 1, pixels: new Uint32Array([16_777_217, 4_000_000_001]) },
  ];
  const maximum = projectStack(uint32Planes, { mode: 'max' }).pixels;
  const sum = projectStack(uint32Planes, { mode: 'sum' }).pixels;
  assert.equal(maximum instanceof Float64Array, true);
  assert.deepEqual([...maximum], [16_777_217, 4_000_000_001]);
  assert.deepEqual([...sum], [33_554_433, 8_000_000_001]);
});
