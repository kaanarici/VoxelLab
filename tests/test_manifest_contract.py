...
def test_manifest_rejects_display_volume_with_mixed_frame_of_reference() -> None:
    manifest = fixture_manifest()
    manifest["series"][0].update(
        {
            "geometryKind": "volumeStack",
    ...
    assert "series[0].frameOfReferenceUIDConsistent: display-volume series must have one consistent FrameOfReferenceUID" in errors


def test_manifest_rejects_display_volume_with_duplicate_slice_positions() -> None:
    manifest = fixture_manifest()
    manifest["series"][0].update(
        {
            "geometryKind": "volumeStack",
    ...
    assert "series[0].slicePositionsDistinct: display-volume series must have distinct patient-space slice positions" in errors
