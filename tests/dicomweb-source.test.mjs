...
const {
  ...
  frameUrl,
  seriesMetadataUrl,
} = await import('../js/dicom/dicomweb/dicomweb-source.js');
const { classifyDICOMImport } = await import('../js/dicom/dicom-import-parse.js');
...
test('frameUrl rejects invalid frame path segments', () => {
  assert.throws(
    () => frameUrl('https://pacs.example/wado-rs', '1.2.study', '1.2.series', '1.2.sop', '../2'),
    /invalid frame number/i,
  );
  assert.throws(
    () => frameUrl('https://pacs.example/wado-rs', '1.2.study', '1.2.series', '1.2.sop', 0),
    /invalid frame number/i,
  );
});
...
test('fetchFrameBytes rejects invalid frame numbers before fetching', async () => {
  await assert.rejects(
    () => fetchFrameBytes({
      wadoBase: 'https://pacs.example/wado-rs',
      studyUID: '1.2.study',
      ...
      frame: '../../2',
      fetchImpl: async () => {
        assert.fail('unexpected fetch for invalid frame number');
      },
    }),
    /invalid frame number/i,
  );
...
test('fetchSeriesMetadata honors HTTP-date Retry-After backoff', async (t) => {
  const previousSetTimeout = globalThis.setTimeout;
  ...
  Date.now = () => Date.parse('Sat, 30 May 2026 12:00:00 GMT');
  globalThis.setTimeout = (fn, delay = 0) => {
    seenDelay = Number(delay || 0);
    fn();
    return 0;
    ...
    fetchImpl: async () => {
      ...
      if (calls === 1) {
        return {
          ok: false,
          status: 503,
          headers: { get: (name) => (name === 'Retry-After' ? 'Sat, 30 May 2026 12:00:05 GMT' : null) },
        };
      }
      return {
        ok: true,
...
test('fetchSeriesItems expands enhanced multi-frame metadata into one shared import item per frame', async () => {
  const metadata = [normalizeInstance(dicomJsonInstance({
    ...
    '00280008': { vr: 'IS', Value: [2] },
    '00281053': { vr: 'DS', Value: [9] },
    '00281052': { vr: 'DS', Value: [-90] },
    '52009229': {
      vr: 'SQ',
      Value: [{
        '00289110': { vr: 'SQ', Value: [{ '00280030': { vr: 'DS', Value: [0.5, 0.5] }, '00180050': { vr: 'DS', Value: [1.0] } }] },
        '00209116': { vr: 'SQ', Value: [{ '00200037': { vr: 'DS', Value: [1, 0, 0, 0, 1, 0] } }] },
        '00289145': { vr: 'SQ', Value: [{ '00281053': { vr: 'DS', Value: [3] }, '00281052': { vr: 'DS', Value: [-30] } }] },
      }],
    },
    '52009230': {
      vr: 'SQ',
      Value: [
        {
          '00209113': { vr: 'SQ', Value: [{ '00200032': { vr: 'DS', Value: [0, 0, 0] } }] },
          '00289145': { vr: 'SQ', Value: [{ '00281053': { vr: 'DS', Value: [2] }, '00281052': { vr: 'DS', Value: [-20] } }] },
        },
        { '00209113': { vr: 'SQ', Value: [{ '00200032': { vr: 'DS', Value: [0, 0, 1] } }] } },
      ],
    },
  ...
  assert.deepEqual(
    items.map((item) => [item.meta.RescaleSlope, item.meta.RescaleIntercept]),
    [[2, -20], [3, -30]],
  );
...
test('fetchSeriesItems treats invalid frame concurrency as conservative', async () => {
  let active = 0;
  let peak = 0;
  const metadata = [normalizeInstance(dicomJsonInstance({
    '00080060': { vr: 'CS', Value: ['US'] },
    '00080018': { vr: 'UI', Value: ['1.2.sop.us.invalid-concurrency'] },
...
test('clearDicomwebSessions drops normalized metadata cache for explicit sessions', async () => {
  clearDicomwebSessions();
  let calls = 0;
  const fetchImpl = async () => {
    calls += 1;
    return {
      ...
      async json() {
        return [dicomJsonInstance({
          '00080060': { vr: 'CS', Value: [calls === 1 ? 'CT' : 'MR'] },
        })];
      },
    };
  };

  const first = await fetchSeriesMetadata({
  ...
  clearDicomwebSessions();
  const second = await fetchSeriesMetadata({
    sessionId: 'reset-session',
    wadoBase: 'https://pacs.example/wado-rs',
    studyUID: '1.2.study',
    seriesUID: '1.2.series',
  ...
  assert.equal(calls, 2);
  assert.equal(first[0].Modality, 'CT');
  assert.equal(second[0].Modality, 'MR');
});

test('normalized DICOMweb metadata cache stays bounded across session churn', async () => {
  clearDicomwebSessions();
  let calls = 0;
  for (let index = 0; index < 110; index += 1) {
    await fetchSeriesMetadata({
      sessionId: `metadata-session-${index}`,
      wadoBase: 'https://pacs.example/wado-rs',
      studyUID: '1.2.study',
      seriesUID: '1.2.series',
      fetchImpl: async () => {
...
test('discoverQidoStudies caches repeated query params without comma-string collisions', async () => {
  clearDicomwebSessions();
  const seenUrls = [];
  const fetchImpl = async (url) => {
    ...
    return {
      ...
      async json() {
        return [{
          '0020000D': { vr: 'UI', Value: [`1.2.study.query.${index}`] },
          '00100010': { vr: 'PN', Value: [{ Alphabetic: `PATIENT^${index}` }] },
        }];
      },
    };
  };

  const repeatedParams = await discoverQidoStudies({
    sessionId: 'qido-study-query-key-session',
    wadoBase: 'https://pacs.example/dicom-web',
    query: { PatientName: ['DOE*', '', 'SMITH*', null] },
    fetchImpl,
  });
  const commaString = await discoverQidoStudies({
    sessionId: 'qido-study-query-key-session',
    wadoBase: 'https://pacs.example/dicom-web',
    query: { PatientName: 'DOE*,SMITH*' },
    fetchImpl,
  });
  const cachedRepeatedParams = await discoverQidoStudies({
    sessionId: 'qido-study-query-key-session',
    wadoBase: 'https://pacs.example/dicom-web',
    query: { PatientName: ['DOE*', '', 'SMITH*', null] },
    fetchImpl: async () => {
      throw new Error('QIDO repeated-param cache miss');
    },
  });

  assert.equal(seenUrls.length, 2);
  assert.deepEqual(new URL(seenUrls[0]).searchParams.getAll('PatientName'), ['DOE*', 'SMITH*']);
  assert.equal(new URL(seenUrls[1]).searchParams.get('PatientName'), 'DOE*,SMITH*');
  assert.equal(repeatedParams[0].studyUID, '1.2.study.query.1');
  assert.equal(commaString[0].studyUID, '1.2.study.query.2');
  assert.deepEqual(cachedRepeatedParams, repeatedParams);
});
...
test('DICOMweb frame cache normalizes equivalent frame numbers', async () => {
  clearDicomwebSessions();
  let calls = 0;
  const wadoBase = 'https://pacs.example/wado-rs';
  const request = {
    sessionId: 'normalized-frame-cache',
    wadoBase,
    studyUID: '1.2.study',
    seriesUID: '1.2.series',
    instanceUID: '1.2.sop.1',
  ...
  const first = await fetchFrameBytes({
    ...request,
    frame: '02',
    fetchImpl: async (url) => {
      calls += 1;
      assert.equal(String(url), `${wadoBase}/studies/1.2.study/series/1.2.series/instances/1.2.sop.1/frames/2`);
      return {
        ok: true,
        async arrayBuffer() {
  ...
  const second = await fetchFrameBytes({
    ...request,
    frame: 2,
    fetchImpl: async () => {
      assert.fail('normalized frame cache should avoid network');
    },
  });

  assert.equal(calls, 1);
  assert.equal(second, first);
  assert.deepEqual(Array.from(new Uint8Array(second)), [2]);
});
...
test('DICOMweb session registry replaces older explicit sessions for one scope', () => {
  clearDicomwebSessions();
  const first = resolveDicomwebImportSession({
    sessionId: 'same-scope-first',
    wadoBase: 'https://pacs.example/wado-rs',
  });
  const second = resolveDicomwebImportSession({
    sessionId: 'same-scope-second',
    wadoBase: 'https://pacs.example/wado-rs',
  });

  assert.equal(getDicomwebSessionStats(first.id), null);
  assert.equal(getDicomwebSessionStats(second.id).sessionId, second.id);
});
