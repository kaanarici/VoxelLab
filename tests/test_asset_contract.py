...
def write_asks_sidecar(root: Path, slug: str) -> None:
    _ = (root / f"{slug}_asks.json").write_text(json.dumps({"slug": slug, "entries": []}))
...
def test_assets_reject_missing_ask_history_sidecar_when_enabled(tmp_path: Path) -> None:
    write_complete_fixture(tmp_path)
    manifest = fixture_manifest()
    manifest["series"][0]["hasAskHistory"] = True

    errors = validate_assets(manifest, tmp_path, exhaustive=True)

    assert "sample: missing sidecar sample_asks.json" in errors


def test_assets_accept_valid_ask_history_sidecar_when_enabled(tmp_path: Path) -> None:
    write_complete_fixture(tmp_path)
    manifest = fixture_manifest()
    series = manifest["series"][0]
    series["hasAskHistory"] = True
    write_asks_sidecar(tmp_path, "sample")
