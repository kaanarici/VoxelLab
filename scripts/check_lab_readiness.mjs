...
export const RESEARCHER_WORKFLOW_EVIDENCE = Object.freeze([
  {
...
const STEP_CLAIMS = Object.freeze({
  ...
  publicExport: 'contract check: sanitized public export removes private workflow files, patient data, local reports, provider config, and repo-only sync scripts while preserving export-repository metadata before one-root publication',
  publicSamples: 'oracle check: public OME-TIFF, ImageJ TIFF, and OME-Zarr sample evidence with explicit support boundaries',
  browserFlows: 'runtime check: browser upload triage with visible unsupported skips, file read failures, and folder read failures, local DICOM/NIfTI source provenance, bounded scalar 4D NIfTI related-timepoint import, mixed native medical/microscopy family boundaries, visible skipped local derived-object reasons, source-bound DICOM SR and microscopy sidecar guidance, MPR geometry stability, microscopy format boundaries, fail-closed sidecar mismatch messages, microscopy C/Z/T time navigation, calibrated microscopy scale bars and measurements, microscopy navigation/workflow replay, analysis workflow with first-use microscopy particle evidence ZIP package export, calibrated export artifacts, and limited OME-Zarr user flows',
