...
from engine_preflight import validate_projection_source, validate_registration_source, validate_ultrasound_source
from modal_contract import INPUT_KINDS, PROCESSING_MODES, default_input_kind, input_kind_error, validate_input_kind, validate_processing_mode
...
def put_file(url: str, path: Path, timeout: int = 120) -> None:
    with path.open("rb") as data:
        req = urllib.request.Request(url, data=cast(Any, data), method="PUT")
...
def candidate_files(folder: Path) -> list[Path]:
    ...
    for path in sorted(folder.rglob("*")):
        if not path.is_file() or is_skipped_path(path, folder):
            continue
        if path.name in SOURCE_MANIFEST_NAMES and path.parent != folder:
            continue
...
def submit(args: argparse.Namespace) -> dict:
    ...
    raw_input_kind = getattr(args, "input_kind", "")
    input_kind = validate_input_kind(raw_input_kind, processing_mode)
    batch_size = max(1, min(int(args.batch_size), MAX_UPLOAD_ITEMS))
    ...
    if not input_kind:
        expected = default_input_kind(processing_mode)
        if input_kind_error(raw_input_kind, processing_mode) != "invalid input_kind" and expected:
            raise SystemExit(f"{processing_mode} requires --input-kind {expected}")
    ...
    if max_wait_seconds < 1:
        raise SystemExit("--max-wait-seconds must be at least 1")
    ...
    if not args.skip_upload and not upload_origin_allowlist:
        raise SystemExit("missing trusted upload origin; set R2_PUBLIC_URL or pass --trusted-upload-origin")
    ...
    if args.skip_upload:
        ...
        uploaded = 0
        for batch_start, batch in enumerate(chunks(files, batch_size)):
            batch_items = upload_items(batch, start_index=batch_start * batch_size)
            response = post_json(
                upload_endpoint,
                {
                    "job_id": args.job_id,
            ...
            with ThreadPoolExecutor(max_workers=upload_workers) as pool:
                ...
                for future in as_completed(futures):
                    ...
                    future.result()
                    uploaded += 1
                    should_report = uploaded % progress_every == 0 or uploaded == len(files)
                    if should_report:
    ...
    while time.time() < deadline:
        ...
        if status.get("status") == "complete":
            if status.get("series_entry"):
                status["series_entry"] = normalize_series_entry(status.get("series_entry"), r2_base, job_id=args.job_id)
                ...
                try:
                    status["series_entry"] = normalize_series_entry(get_json(series_url), r2_base, job_id=args.job_id)
