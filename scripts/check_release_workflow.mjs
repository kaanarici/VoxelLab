...
requireText('npx playwright install chromium', 'release lab readiness must install the browser used by Playwright proof');
requireText('xvfb-run -a env PYTHON=python npm run check:lab -- --report lab-readiness-report.json --skip-validation-matrix --skip-public-export --skip-demo-pack --skip-converters', 'release workflow must produce a lab-readiness report with the public-release proof gate');
...
console.log('OK: release workflow contract verified — its YAML requires lab readiness, desktop builds, release asset validation, and downloadable artifacts (static inspection; the steps run in CI, not here)');
