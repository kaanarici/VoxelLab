...
from engine_preflight import (
    dicom_frame_count,
    source_dicom_files,
    validate_projection_source,
    validate_registration_source,
    validate_ultrasound_source,
)
...
def validate_ct_pipeline(source: Path | None, slugs: list[str]) -> list[str]:
    ...
    can_read_dicom = not any(error.endswith("pydicom") for error in errors)
...
def main() -> int:
    ...
    _ = parser.add_argument("--registration-source", action="append", default=[], metavar="DIR", help="Registration source folder to validate")
    ...
    try:
        ...
        for raw in args.registration_source:
            errors.extend(validate_registration_source(Path(raw).expanduser().resolve()))
