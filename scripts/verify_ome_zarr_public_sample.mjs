...
async function main() {
  ...
  try {
    ...
    console.log(JSON.stringify({
      ...
      loadedLevel: parsed.results[0].entry.microscopy?.storageProvenance || '',
      boundary: 'bounded coarsest-level local proof only: this fixture omits chunk payloads, relies on declared zero fill_value, and still keeps full-resolution level-0 eager import over budget',
    }, null, 2));
  } finally {
