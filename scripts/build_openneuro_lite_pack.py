...
import analyze
import context
from scripts.install_demo_data import download_to_path
...
MAX_OPENNEURO_SOURCE_BYTES = 2 * 1024 * 1024 * 1024
...
def build_series(temp_data: Path, spec: dict[str, Any], download_dir: Path) -> dict[str, Any]:
    ...
    _ = download_to_path(spec["url"], nii_path, max_bytes=MAX_OPENNEURO_SOURCE_BYTES)
    bval_path = None
    if spec.get("bval_url"):
        bval_path = download_dir / f"{slug}.bval"
        _ = download_to_path(spec["bval_url"], bval_path, max_bytes=MAX_OPENNEURO_SOURCE_BYTES)
