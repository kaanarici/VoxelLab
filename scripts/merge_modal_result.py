...
from series_contract import merge_manifest_path, merge_manifest_series
...
def read_result_json(source: str, timeout: int = 30, max_bytes: int = MAX_RESULT_JSON_BYTES) -> dict[str, Any]:
    ...
    if parsed.scheme in {"http", "https"}:
        ...
        if path.stat().st_size > max_bytes:
            raise ValueError(f"series result: JSON exceeds {max_bytes} bytes")
        raw = path.read_bytes()
...
def companion_projection_source(source: str) -> str:
    ...
    if parsed.scheme in {"http", "https"} and parsed.path.endswith("/series.json"):
        path = parsed.path[:-len("/series.json")] + "/projection_set.json"
...
def read_companion_projection(source: str, timeout: int = 30) -> dict[str, Any] | None:
    ...
    if not projection_source:
        return None
    try:
        return read_result_json(projection_source, timeout=timeout)
...
def main() -> int:
    ...
    projection_entry = read_companion_projection(source, timeout=args.timeout)
    ...
    if args.dry_run:
        ...
        def merge(current: dict[str, Any]) -> dict[str, Any]:
            manifest, action, index = merge_manifest_series(
                current,
                entry,
                projection_entry=projection_entry,
                job_id=job_id,
            ...
            result.update({"action": action, "index": index})
            return manifest

        manifest = update_json(manifest_path, merge)
